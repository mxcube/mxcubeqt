...
class PX2MultiCollect(SOLEILMultiCollect):
    ...
    def collect_trajectory_image(self, image):
        self.logger.info('Collecting trajectory image %s' % image)
        self.set_position(image['position'])
        self.set_mono_energy(image['energy'])
        self.set_transmission(image['transmission'])
        self.set_detector_position(image['detector_position'])
        
        filename = image['filename']
        start_angle = image['start_angle']
        while self.mono_mt_rx.state().name != 'OFF':
            self.safeTurnOff(self.mono_mt_rx)
            time.sleep(0.1)
        print 'taking image %s at %s' % (filename, start_angle)
        
        self.collectImage(filename, start_angle)
    ...
    def collectImage(self, fileName, ScanStartAngle):
        ...
        while imageTaken is False and self.stt not in ['STOP', 'ABORT']:
            try:
                ...
                self.md2.OmegaPosition = ScanStartAngle - 0.1
                self.wait()
                
                self.wait(self.adsc)
                head = self.setupHeader(X=self.X, Y=self.Y, D=self.D)
                ...
                if self.master == 'md2':
                    self.limaadscSnap()
                    self.StartScan(fileName)
                    self.limaadscStop()
                elif self.master == 'cats':
                    self.StartScan(fileName)
        ...
        if self.synchronize is True:
            self.synchronize_thread(fileName)
        self.analyze_thread(fileName)
    ...
    def do_collect(self, owner, data_collect_parameters, in_multicollect=False):
        ...
        reference_interval = oscillation_parameters.get("reference_interval", 1)
        if inverse_beam:
            inverse_exp = reference_interval
        else:
            inverse_exp = False
        
        def get_interleaving_wedge_size_from_prefix(prefix):
            if 'interleave' in prefix:
                i = prefix.index('interleave')
                p = prefix[i:]
                ps = p.split('_')
                interval = int(ps[1])
                return interval
        ...
        interleave = get_interleaving_wedge_size_from_prefix(file_parameters["prefix"])
        
        self.experiment = Experiment(ScanStartAngle,
                                     nbFrames*ScanRange, #sweep size
                                     ScanRange,
                                     first_image=firstImage,
                                     directory=file_parameters["directory"],
                                     prefix=file_parameters["prefix"],
                                     run=file_parameters["run_number"],
                                     transmissions=[data_collect_parameters["transmission"]],
                                     detector_distances=[oscillation_parameters["detdistance"]],
                                     energies=[data_collect_parameters["energy"]],
                                     inverse=reference_interval,
                                     interleave=interleave,
                                     helical=self.helical,
                                     translational=self.translational,
                                     grid=self.grid,
                                     interleave_raster=False,
                                     inverse_raster=False,
                                     grid_raster=True)
        
        trajectory = self.experiment.get_trajectory()
        ...
        with cleanup(self.data_collection_cleanup):
            ...
            for image in trajectory:
                self.collect_trajectory_image(image)
