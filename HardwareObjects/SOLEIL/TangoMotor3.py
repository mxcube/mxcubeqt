...
#        logging.getLogger("HWR").info("%s: TangoMotor._init, %s", self.name(), '')
    ...
    def motorState(self):
        return TangoMotor3.stateDict[str(self.device.getState())]
    ...
    def syncMove(self, position):
        ...
        print 'move started from %s to %s, state is %s' % (prev_position, position, self.device.getState())
        
        while self.device.getState() == "RUNNING" or self.device.getState() == "MOVING": # or str(self.device.State()) == SpecMotor.MOVESTARTED:
    ...
    def syncMoveRelative(self, position):
        ...
        while self.device.getState() == "RUNNING" or self.device.getState() == "MOVING":
            qApp.processEvents(100)
