...
class BlissSplashScreen(qt.QSplashScreen):
    ...
    def drawContents(self, painter):
        ...
        painter.drawText(qt.QRect(qt.QPoint(x0, y0), qt.QPoint(x1, y1)), qt.Qt.AlignLeft | qt.Qt.AlignTop, 'Loading')
        painter.font().setPixelSize(pxsize*2.5)
        y0 = y1
        y1 += 3 + painter.fontMetrics().height()
        painter.drawText(qt.QRect(qt.QPoint(x0, y0), qt.QPoint(x1, y1)), qt.Qt.AlignCenter, self.guiName)
        painter.font().setPixelSize(pxsize)
        y0 = y1
        y1 += 3 + painter.fontMetrics().height()
        painter.drawText(qt.QRect(qt.QPoint(x0, y0), qt.QPoint(x1, y1)), qt.Qt.AlignLeft | qt.Qt.AlignBottom, 'Please wait...')
...
class GUISupervisor(qt.QWidget):
    def __init__(self, executionOnly = False, designMode = False, showMaximized=False, noBorder=False):
        ...
        self.windows = []
        self.splashScreen = BlissSplashScreen(Icons.load('splash'), qt.Qt.WDestructiveClose)
        ...
        if len(self.windows) > 0:
                        ...
                        for connection in item["connections"]:
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    sender.connect(sender, qt.PYSIGNAL(connection["signal"]), slot)
            ...
            for window in self.windows:
                ...
                for key in [qt.Qt.Key_F9,qt.Qt.Key_F10,qt.Qt.Key_F11,qt.Qt.Key_F12] :
                    window._splitterPositionAccel.insertItem(qt.Qt.SHIFT+key,qt.Qt.SHIFT+key)
                    window._splitterPositionAccel.insertItem(qt.Qt.CTRL+key,qt.Qt.CTRL+key)
                    window._splitterPositionAccel.insertItem(key,key)

                qt.QObject.connect(window._splitterPositionAccel,qt.SIGNAL('activated(int)'),self.saveOrReloadSize)
                qt.QObject.connect(window._fontSizeAccel, qt.SIGNAL('activated(int)'), self.changeFontSize)
                qt.QObject.connect(window._whatsThisAccel, qt.SIGNAL('activated(int)'), BlissWidget.updateWhatsThis)
                qt.QObject.connect(window._reloadAccel, qt.SIGNAL('activated(int)'), self.reloadGUI)
