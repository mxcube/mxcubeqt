...
class ESRFConfigurationBrick(BaseWidget):
    ...
    def __init__(self, *args):
        ...
        self.ui_widgets_manager = qt_import.load_ui_file("esrf_id13_configuration_widget.ui")
    ...
    def beam_pos_cal_data_changed(self, who_changed, new_data_dict):
        ...
        for row_index in range(table.rowCount()):
            if table.item(row_index, 0).text() == current_pos_name:
                if who_changed == 0:
                    table.item(row_index, 1).setBackground(qt_import.QColor(qt_import.Qt.yellow))
                    table.item(row_index, 2).setBackground(qt_import.QColor(qt_import.Qt.yellow))
                elif who_changed == 1:
                    table.item(row_index, 3).setBackground(qt_import.QColor(qt_import.Qt.yellow))
                    table.item(row_index, 4).setBackground(qt_import.QColor(qt_import.Qt.yellow))
    ...
    def fill_config_table(self):
        ...
        if tmp_dict is not None:
            ...
            self.ui_widgets_manager.configuration_table.horizontalHeader().setSectionResizeMode(
                qt_import.QHeaderView.ResizeToContents
            )
