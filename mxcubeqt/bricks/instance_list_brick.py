...
WANTS_CONTROL_EVENT = qt_import.QEvent.User


class WantsControlEvent(qt_import.QEvent):
    def __init__(self, client_id):
        qt_import.QEvent.__init__(self, WANTS_CONTROL_EVENT)
        self.client_id = client_id


START_SERVER_EVENT = qt_import.QEvent.User + 1


class StartServerEvent(qt_import.QEvent):
    def __init__(self):
        qt_import.QEvent.__init__(self, START_SERVER_EVENT)


APP_BRICK_EVENT = qt_import.QEvent.User + 2


class AppBrickEvent(qt_import.QEvent):
    def __init__(self, brick_name, widget_name, method_name, method_args, master_sync):
        qt_import.QEvent.__init__(self, APP_BRICK_EVENT)
...
APP_TAB_EVENT = qt_import.QEvent.User + 3


class AppTabEvent(qt_import.QEvent):
    def __init__(self, tab_name, tab_index):
        qt_import.QEvent.__init__(self, APP_TAB_EVENT)
        self.tab_name = tab_name
        self.tab_index = tab_index


MSG_DIALOG_EVENT = qt_import.QEvent.User + 4


class MsgDialogEvent(qt_import.QEvent):
    def __init__(self, icon_type, msg, font_size, callback=None):
        qt_import.QEvent.__init__(self, MSG_DIALOG_EVENT)
...
USER_INFO_DIALOG_EVENT = qt_import.QEvent.User + 5


class UserInfoDialogEvent(qt_import.QEvent):
    def __init__(self, msg, fromaddrs, toaddrs, subject, is_local, font_size):
        qt_import.QEvent.__init__(self, USER_INFO_DIALOG_EVENT)
        self.msg = msg
...
class InstanceListBrick(BaseWidget):
    ...
    def __init__(self, *args):
        ...
        # Graphic elements ----------------------------------------------------
        _main_widget = qt_import.QWidget(self)
        _main_gbox = qt_import.QGroupBox("Current users", self)

        self.users_listwidget = qt_import.QListWidget(_main_gbox)
        self.users_listwidget.setFixedHeight(50)

        self.give_control_chbox = qt_import.QCheckBox(
            "Selecting gives control", _main_gbox
        )
        self.give_control_chbox.setChecked(False)
        self.allow_timeout_control_chbox = qt_import.QCheckBox(
            "Allow timeout control", _main_gbox
        )
        self.allow_timeout_control_chbox.setChecked(False)

        # self.take_control_button = qt_import.QToolButton(_main_gbox)
        # self.take_control_button.setUsesTextLabel(True)
        self.take_control_button = qt_import.QToolButton(_main_gbox)
        self.take_control_button.setToolButtonStyle(
            qt_import.Qt.ToolButtonTextBesideIcon
        )
        # self.take_control_button.setUsesTextLabel(True)
        ...
        self.ask_control_button = qt_import.QToolButton(_main_gbox)
        # self.ask_control_button.setUsesTextLabel(True)
        # self.ask_control_button = qt_import.QtGui.qt_import.QToolButton(_main_gbox)
        self.ask_control_button.setToolButtonStyle(qt_import.Qt.ToolButtonTextBesideIcon)
        # self.ask_control_button.setUsesTextLabel(True)
        ...
        _my_name_widget = qt_import.QWidget(_main_gbox)
        _my_name_label = qt_import.QLabel("My name:", _my_name_widget)
        self.nickname_ledit = NickEditInput(_my_name_widget)

        reg_exp = qt_import.QRegExp(".+")
        nick_validator = qt_import.QRegExpValidator(reg_exp, self.nickname_ledit)
        self.nickname_ledit.setValidator(nick_validator)

        self.external_user_info_dialog = ExternalUserInfoDialog()

        # Layout --------------------------------------------------------------
        _my_name_widget_layout = qt_import.QHBoxLayout(_my_name_widget)
        ...
        _main_widget_vlayout = qt_import.QVBoxLayout(_main_widget)
        ...
        _main_gbox_vlayout = qt_import.QVBoxLayout(_main_gbox)
        ...
        # Other ---------------------------------------------------------------
        self.timeout_timer = qt_import.QTimer(self)
    ...
    def client_initialized(
        ...
        if connected is None:
            ...
            if not quiet:
                ...
                qt_import.QApplication.postEvent(self, msg_event)

            qt_import.QTimer.singleShot(
            ...
            item = qt_import.QListWidgetItem(
            ...
            item.setFlags(qt_import.Qt.ItemIsEnabled)
    ...
    def server_initialized(self, started, server_id=None):
        if started:
            ...
            msg_dialog.setButtonText(qt_import.QMessageBox.Ok, "qt_import.Quit")
    ...
    def application_tab_changed(self, tab_name, tab_index):
        if self.instance_server_hwobj is not None:
            ...
            qt_import.QApplication.postEvent(self, tab_event)
    ...
    def application_brick_changed(
        ...
        if not master_sync or self.instance_server_hwobj is not None:
            ...
            qt_import.QApplication.postEvent(self, brick_event)
    ...
    def take_control_clicked(self):
        ...
        take_control_dialog = qt_import.QMessageBox(
            "mxCuBE",
        ...
        take_control_dialog.setButtonText(qt_import.QMessageBox.Ok, "Take control")
        # s=self.font().pointSize()
        # f=take_control_dialog.font()
        # f.setPointSize(s)
        # take_control_dialog.setFont(f)
        # take_control_dialog.updateGeometry()
        if take_control_dialog.exec_() == qt_import.QMessageBox.Ok:
            self.instance_server_hwobj.takeControl()

    def run(self):
        if self["initializeServer"]:
            qt_import.QApplication.postEvent(self, StartServerEvent())
    ...
    def have_control(self, have_control, gui_only=False):
        ...
        if have_control:
            ...
            self.users_listwidget.setSelectionMode(
                qt_import.QAbstractItemView.SingleSelection
            )
            ...
            for item_index in range(self.users_listwidget.count()):
                ...
                # tem.setFlags(qt_import.QtCore.qt_import.Qt.NoItemFlags)
            self.users_listwidget.setSelectionMode(
                qt_import.QAbstractItemView.NoSelection
            )
        else:
            if self.xmlrpc_server:
    ...
    def pass_control(self, has_control_id):
        try:
            ...
            control_item.setFlags(
                qt_import.Qt.ItemIsEnabled | qt_import.Qt.ItemIsSelectable
            )
    ...
    def timeout_approaching(self):
        if self.give_control_msgbox is not None:
            ...
            if self.timeout_left == 0:
                self.give_control_msgbox.done(qt_import.QMessageBox.Yes)
            else:
                self.give_control_msgbox.setButtonText(
                    qt_import.QMessageBox.Yes, "Allow (%d secs)" % self.timeout_left
                )
        ...
        if self.is_running():
            if event.type() == WANTS_CONTROL_EVENT:
                try:
                    ...
                    self.give_control_msgbox.setButtonText(
                        qt_import.QMessageBox.No, "Deny"
                    )
                    self.timeout_timer.start(1000)
                    res = self.give_control_msgbox.exec_()
                    self.timeout_timer.stop()

                    if res == qt_import.QMessageBox.Yes:
                ...
                if event.toaddrs != "":
                    ...
                    try:
                        ...
                        smtp.quit()
        return qt_import.QWidget.event(self, event)
    ...
    def wants_control(self, client_id):
        if (
            ...
            client_print = self.instance_server_hwobj.idPrettyPrint(client_id)
            self.give_control_msgbox = qt_import.QMessageBox(
                "Pass control",
            ...
            qt_import.QApplication.postEvent(self, custom_event)


class LineEditInput(qt_import.QLineEdit):
    """
    ...
    PARAMETER_STATE = {
        "INVALID": qt_import.Qt.red,
        "OK": qt_import.Qt.white,
        "WARNING": qt_import.Qt.yellow,
    }

    inputValidSignal = qt_import.pyqtSignal(bool)
    ...
    def __init__(self, parent):
        ...
        self.origPalette = qt_import.QPalette(self.palette())
        self.palette2 = qt_import.QPalette(self.origPalette)
        self.palette2.setColor(
            qt_import.QPalette.Active,
            qt_import.QPalette.Base,
            self.origPalette.brush(
                qt_import.QPalette.Disabled, qt_import.QPalette.Background
            ).color(),
        )
        self.palette2.setColor(
            qt_import.QPalette.Inactive,
            qt_import.QPalette.Base,
            self.origPalette.brush(
                qt_import.QPalette.Disabled, qt_import.QPalette.Background
            ).color(),
    ...
    def text(self):
        return str(qt_import.QLineEdit.text(self))
    ...
    def sizeHint(self):
        size_hint = qt_import.QLineEdit.sizeHint(self)
    ...
    def setReadOnly(self, readonly):
        ...
        qt_import.QLineEdit.setReadOnly(self, readonly)
...
class NickEditInput(LineEditInput):
    ...
    def acceptInput(self):
        colors.set_widget_color(
            self, LineEditInput.PARAMETER_STATE["OK"], qt_import.QPalette.Base
        )


class ExternalUserInfoDialog(qt_import.QDialog):
    def __init__(self):
        qt_import.QDialog.__init__(self, None)
        ...
        # Graphic elements ----------------------------------------------------
        self.message_box = qt_import.QGroupBox("Your info", self)  # v
        self.message1 = qt_import.QLabel(self.message_box)
        self.message2 = qt_import.QLabel(
            "<nobr>Please enter the following "
            + "information, <u>required</u> for "
            + "the experimental hall operator:",
            self.message_box,
        )
        my_name_widget = qt_import.QWidget(self.message_box)
        name_label = qt_import.QLabel("Your name:", my_name_widget)
        self.name_input = qt_import.QLineEdit(my_name_widget)
        institute_label = qt_import.QLabel("Your institute:", my_name_widget)
        self.institute_input = qt_import.QLineEdit(my_name_widget)
        phone_label = qt_import.QLabel("Your telephone:", my_name_widget)
        self.phone_input = qt_import.QLineEdit(my_name_widget)
        email_label = qt_import.QLabel("Your email:", my_name_widget)
        self.email_input = qt_import.QLineEdit(my_name_widget)
        ...
        # Other ---------------------------------------------------------------
        _my_name_widget_vlayout = qt_import.QVBoxLayout(my_name_widget)
        ...
        _message_gbox_vlayout = qt_import.QVBoxLayout(self.message_box)
    ...
    def show(self):
        ...
        return qt_import.QDialog.exec_(self)
