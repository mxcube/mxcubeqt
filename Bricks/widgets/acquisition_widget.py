...
class AcquisitionWidget(qt.QWidget):
    ...
    def set_beamline_setup(self, beamline_setup):
        ...
        if 'osc_range' in limits_dict:
            ...
            (lower, upper, prec) = limits
            osc_start_validator = qt.QDoubleValidator(lower, upper, prec, self)
            osc_range_validator = qt.QDoubleValidator(lower, upper, prec, self)
        else:
            osc_start_validator = qt.QDoubleValidator(0.0001, 1000, 4, self)
            osc_range_validator = qt.QDoubleValidator(0.0001, 1000, 4, self)
        ...
        if 'exposure_time' in limits_dict:
            ...
            (lower, upper, prec) = limits
            exp_time_valdidator = qt.QDoubleValidator(lower, upper, prec, self)
        else:
