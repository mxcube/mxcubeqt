...
from widgets.radiation_damage_char_widget_layout import \
...
class CharTypeWidget(QWidget):
    def __init__(self,parent = None,name = None,fl = 0):
        ...
        # Logic
        #
        QObject.connect(self.routine_dc_page.dose_limit_cbx,
                        SIGNAL("toggled(bool)"),
                        self.enable_dose_ledit)

        QObject.connect(self.routine_dc_page.time_limit_cbx,
                        SIGNAL("toggled(bool)"),
                        self.enable_time_ledit)

        QObject.connect(self.routine_dc_page.dose_time_bgroup, 
                        SIGNAL("clicked(int)"),
                        self._toggle_time_dose)

        self._toggle_time_dose(self.routine_dc_page.\
                                   dose_time_bgroup.selectedId())
