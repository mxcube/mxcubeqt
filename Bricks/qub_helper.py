...
class QubHelper(object):
    __qub_helper_instance = None

    def __init__(self):
        object.__init__(self)
        
        if not QubHelper.__qub_helper_instance:
            QubHelper.__qub_helper_instance = _QubHelper()

    def get_instance(self):
        return QubHelper.__qub_helper_instance



class _QubHelper(object):
    def __init__(self):
        self.drawing = None
        self.view = None
        self.drawing_event = None

        # Helical paths (lines) stored as 5-tuples
        # <starting QubPoint, ending_QubPoint, QubLine, QTextListItem,
        # starting CPositon, ending CPositon>
        #self.qub_lines = {}
    ...
    def clear_all(self):
        if self.drawing_event:
            self.drawing_event.de_select_all()
...
class Line(Shape):    
    ...
    def draw(self):
        ...
        try:                
            qub_line, _ = QubAddDrawing(self.drawing, QubLineDrawingMgr, 
...
class Point(Shape):
    ...
    def draw(self, screen_pos):
        ...
        try:
            qub_point, _ = QubAddDrawing(self.drawing, QubPointDrawingMgr, 
                                          QubCanvasTarget)
            qub_point.show()
