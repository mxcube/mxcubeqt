...
class TreeBrick(BaseComponents.BlissWidget):
        ...
        if not logged_in:
            ...
            if sc_basket_content and sc_sample_content:
                ...
                if sc_basket_content:
                    ...
                    self.dc_tree_widget.populate_list_view(
    ...
    def logged_in(self, logged_in):
        ...
        if not logged_in:
            ...
            if (
                self.dc_tree_widget.beamline_setup_hwobj.diffractometer_hwobj.in_plate_mode()
            ):
                ...
                if plate_sample_content:
                    self.dc_tree_widget.beamline_setup_hwobj.set_plate_mode(True)
                    plate_row_list, plate_sample_list = self.dc_tree_widget.samples_from_plate_content(
                        plate_sample_content
                    )
                    self.dc_tree_widget.populate_plate_view(
                        plate_row_list, plate_sample_list
                    )
    ...
    def refresh_sample_list(self):
        ...
        samples = lims_client.get_samples(
        ...
        if samples:
            ...
            sc_basket_list, sc_sample_list = self.dc_tree_widget.samples_from_sc_content(
                sc_basket_content, sc_samples_content
            )
                ...
                if lims_sample:
                    ...
                    if lims_sample:
                        ...
                        if lims_sample:
                            ...
                            logging.getLogger("user_level_log").warning(
                                "No sample in ISPyB for location %s"
                                % str(sc_sample.location)
                            )
                            sample_list.append(sc_sample)
    ...
    def get_sc_content(self):
        ...
        try:
            for basket in self.sample_changer_hwobj.getBasketList():
                ...
                # basket_code, is_present))
                sc_basket_content.append((basket_index + 1, basket))
            ...
            for sample in self.sample_changer_hwobj.getSampleList():
                ...
                sc_sample_content.append(
                    (matrix, basket_index, vial_index, basket_code, 0, coords)
                )
        except BaseException:
            logging.getLogger("user_level_log").info(
                "Could not connect to sample changer,"
    ...
    def get_plate_content(self):
        ...
        for sample in self.plate_manipulator_hwobj.getSampleList():
            ...
            matrix = sample.getID() or ""
            vial_index = ":".join(map(str, coords[1:]))
            plate_sample_content.append(
                (matrix, row_index, col_index, coords[1], 0, coords)
            )
    ...
    def sample_load_state_changed(self, state, *args):
        ...
        self.sample_changer_widget.child("details_button").setPaletteBackgroundColor(
            qt.QColor(s_color)
        )
    ...
    def populate_parameters_tab(self, item=None):
        self.emit(qt.PYSIGNAL("populate_parameter_widget"), (item,))
    ...
    def populate_edna_parameters_tab(self, item):
        self.emit(qt.PYSIGNAL("populate_edna_parameter_widget"), (item,))
    ...
    def toggle_sample_changer_tab(self):
        if self.current_view == self.sample_changer_widget:
            ...
            self.emit(qt.PYSIGNAL("hide_sample_changer_tab"), (True,))
            self.dc_tree_widget.sample_list_view_selection()
            self.sample_changer_widget.child("details_button").setText(
                "Show SC-details"
            )
