...
class MotorWPredefinedPositionsBrick(BlissWidget):
    def __init__(self, *args):
        ...
        #
        QObject.connect(
            self.lstPositions, SIGNAL("activated( int )"), self.lstPositionsClicked
        )
        QObject.connect(
    ...
    def cmdSetPositionClicked(self):
        self.motor.setNewPredefinedPosition(
            str(self.txtPositionName.text()), self.motor.getPosition()
        )
        self.fillPositions()
    ...
    def setMnemonic(self, mne):
        ...
        if self.motor is not None:
            self.disconnect(
                self.motor, PYSIGNAL("stateChanged"), self.motorStateChanged
            )
            self.disconnect(
                self.motor, PYSIGNAL("newPredefinedPositions"), self.fillPositions
            )
            self.disconnect(
                self.motor,
                PYSIGNAL("predefinedPositionChanged"),
                self.predefinedPositionChanged,
            )
