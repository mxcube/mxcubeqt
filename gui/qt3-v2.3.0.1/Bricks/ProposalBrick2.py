...
class ProposalBrick2(BlissWidget):
    ...
    def login(self):
        ...
        if not login_ok:
            ...
            # login succeed but without a scheduled session, a newSession is created
            # instead. Ask the user to accep the new session
            ...
            self.acceptLogin(
                ...
                loginRes["person"],
                loginRes["laboratory"],
                loginRes["session"]["session"],
                loginRes["local_contact"],
            )
