    ...
    def propertyChanged(self, property, oldValue, newValue):
        if property == "resolution":
            if self.resolutionMotor is not None:
                self.disconnect(
                    self.resolutionMotor, PYSIGNAL("deviceReady"), self.resolutionReady
                )
                self.disconnect(
                    self.resolutionMotor,
                    PYSIGNAL("deviceNotReady"),
                    self.resolutionNotReady,
                )
                self.disconnect(
                    self.resolutionMotor,
                    PYSIGNAL("stateChanged"),
                    self.resolutionStateChanged,
                )
                self.disconnect(
                    self.resolutionMotor,
            ...
            if self.detectorMotor is not None:
                self.disconnect(
                    self.detectorMotor, PYSIGNAL("deviceReady"), self.resolutionReady
                )
                self.disconnect(
                    self.detectorMotor,
                    PYSIGNAL("deviceNotReady"),
                ...
                self.disconnect(
                    self.detectorMotor,
                    PYSIGNAL("limitsChanged"),
                    self.detectorLimitsChanged,
                )
            if self.energyHObj is not None:
                self.disconnect(
                    self.energyHObj, PYSIGNAL("moveEnergyFinished"), self.energyChanged
                )
            self.units.clear()
            self.angHistory = []
            self.mmHistory = []
            ...
            if self.resolutionMotor is not None:
                ...
                try:
                    ...
                    logging.getLogger().exception(
                        "ResolutionBrick: problem connecting to the resolution motor"
                    )
                try:
                    self.connect(
                        self.detectorMotor,
                    ...
                    logging.getLogger().exception(
                        "ResolutionBrick: problem connecting to the detector distance motor"
                    )
                try:
                    self.connect(
                        self.energyHObj,
                        PYSIGNAL("moveEnergyFinished"),
                        self.energyChanged,
                    )
                except BaseException:
                    logging.getLogger().exception(
                        "ResolutionBrick: problem connecting to the energy motor"
                    )
        ...
        try:
            ...
            if unit == chr(197):
                if self.resolutionLimits is not None:
                    if (
                        val >= self.resolutionLimits[0]
                        and val <= self.resolutionLimits[1]
                    ):
    ...
    def resolutionStateChanged(self, state):
        if self.detectorMotor is not None:
            ...
            if unit == chr(197):
                ...
                if (
                    state == self.detectorMotor.MOVING
                    or state == self.detectorMotor.MOVESTARTED
                ):
