...
class SoleilMachCurrentBrick(BaseComponents.BlissWidget):
    STATES = {"unknown": QWidget.darkGray, "ready": QWidget.blue, "error": QWidget.red}
    ...
    def propertyChanged(self, propertyName, oldValue, newValue):
        if propertyName == "mnemonic":
            if self.machCurrent is not None:
                self.disconnect(
                    self.machCurrent, PYSIGNAL("valueChanged"), self.setValue
                )
            ...
            if self.machCurrent is not None:
                ...
                if ret:
                    ...
                    self.setValue(
                        value=mach, opmsg=opmsg, fillmode=fillmode, refill=refill
                    )
