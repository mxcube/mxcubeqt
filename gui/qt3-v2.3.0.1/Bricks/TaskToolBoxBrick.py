...
class TaskToolBoxBrick(BaseComponents.BlissWidget):
    ...
    def run(self):
        ...
        self.shape_history.get_drawing_event_handler().deletion_cb = self.shape_deleted

        try:
            self.shape_history.get_drawing_event_handler().move_to_centred_position_cb = (
                self.diffractometer_hwobj.moveToCentredPosition
            )
            self.shape_history.get_drawing_event_handler().move_to_screen_position_cb = (
                self.diffractometer_hwobj.moveToBeam
            )
