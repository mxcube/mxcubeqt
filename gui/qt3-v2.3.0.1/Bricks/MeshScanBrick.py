...
class MeshScanBrick(BaseGraphicScan):
    def __init__(self, parent, name, **kwargs):
        BaseGraphicScan.__init__(
            self, parent, name, uifile="Mesh.ui", cmd_name="mesh", **kwargs
        )
        ...
        qt.QObject.connect(
            meshTable, qt.SIGNAL("valueChanged(int,int)"), self._valueChangedScanParam
        )
    ...
    def _viewConnect(self, view):
        self._graphicSelection = Qub2PointSurfaceDrawingMgr(
    ...
    def _moveGraphicInBeam(self, beamx, beamy):
        ...
        self._graphicSelection.setRect(
            self.__rect.x(), self.__rect.y(), self.__rect.width(), self.__rect.height()
        )
            ...
            if self.__rect is not None:
                ...
                if valid:
                    ...
                    if row == 0:
                        ...
                        if column == 1:
                            self.__rect.setLeft(pix)
                        else:
                            self.__rect.setRight(pix)
                    else:
                        ...
                        if column == 1:
                            self.__rect.setTop(pix)
                        else:
                            self.__rect.setBottom(pix)
                    self._graphicSelection.setRect(
                        self.__rect.x(),
                        self.__rect.y(),
                        self.__rect.width(),
