...
class GraphBrick(BlissWidget):
    def __init__(self, *args):
        ...
        QObject.connect(
            self.cmdEditCurve,
            SIGNAL('clicked()'),
            self.cmdEditCurveClicked)
        QObject.connect(
            self.cmdRemoveCurve,
            SIGNAL('clicked()'),
            self.cmdRemoveCurveClicked)
        QObject.connect(
            self.lstCurves,
            SIGNAL('highlighted(const QString &)'),
            self.curveSelected)
        QObject.connect(
            self.chkXAxisTimeAxis,
            SIGNAL('clicked()'),
    ...
    def cmdSaveDataClicked(self):
        try:
            ...
            logging.getLogger().exception(
                "An error occured while trying to open file %s",
                self["filename"])
            QMessageBox.warning(
            ...
            for curve_name, curve_data in self.curveData.iteritems():
                ...
                contents.append(
                    "#L  %s  %s" %
                    (self.graphWidget.xlabel() or (
                        self.timeAxisX and "time (s.)" or "X"),
                        self.graphWidget.ylabel() or "Y"))
            ...
            try:
                try:
                    ...
                    QMessageBox.information(
                        self, "Success", "Data have been saved successfully to\n%s" %
                        self["filename"], QMessageBox.Ok)
            finally:
                f.close()
    ...
    def run(self):
        ...
        for curve_name, curve_data in self.curveData.items():
            self.graphWidget.newcurve(
                curve_name,
                curve_data.x,
                curve_data.y,
                maptoy2=curve_data.maptoy2)
    ...
    def toggleXAxisTimeAxis(self):
        ...
        for curve_data in self.curveData.itervalues():
            curve_data.clear()
            self.graphWidget.newcurve(str(curve_data.name()),
                                      x=[], y=[], maptoy2=curve_data.maptoy2)
    ...
    def updateCurve(self, curve_setup_dict):
        ...
        if not curve_name in self.graphWidget.curves:
            ...
            QObject.connect(
                self.curveData[curve_name],
                PYSIGNAL('addPoint'),
                self.addPoint)

        if curve_name in self.hardwareObjects and str(
                self.hardwareObjects[curve_name].name()) != mnemonic:
            self.disconnect(
                self.hardwareObjects[curve_name],
                PYSIGNAL('valueChanged'),
                self.curveData[curve_name].addPoint)
            self.disconnect(
                self.hardwareObjects[curve_name],
                PYSIGNAL('timeout'),
                self.curveData[curve_name].timeout)
        ...
        self.graphWidget.newcurve(
            curve_name,
            x=self.curveData[curve_name].x,
            y=self.curveData[curve_name].y,
            maptoy2=maptoy2)
    ...
    def addPoint(self, curve_name, y=None, x=None, timeout=False, replot=True):
        ...
        if self.timeAxisX:
            if self.timeAxisElapsedTime:
                ...
                t = int(time.strftime("%S")) + int(time.strftime("%H")) * \
                    3600 + int(time.strftime("%M")) * 60
                curveData.t = t
    ...
    def cmdEditCurveClicked(self):
        ...
        self.curveSourceSelector.setCurve(
            self.propertyBag[selected_curve],
            canChangeName=False)

        if self.curveSourceSelector.exec_loop() == QDialog.Accepted:
            self.propertyBag.getProperty(selected_curve).setValue(
                self.curveSourceSelector.getCurveSetup())
