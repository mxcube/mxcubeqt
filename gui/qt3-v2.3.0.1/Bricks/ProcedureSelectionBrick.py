...
class ProcedureSelectionBrick(BaseComponents.BlissWidget):
    def __init__(self, *args):
        ...
        self.procedureSelectorBox.layout().addWidget(
            self.equipmentBox, 0, Qt.AlignLeft | Qt.AlignTop
        )
        self.procedureSelectorBox.layout().addWidget(
            self.procedureBox, 0, Qt.AlignRight | Qt.AlignTop
        )
        self.procedureSelectorBox.layout().addItem(
    ...
    def procedureChanged(self, index):
        ...
        if index > 0 and len(self.proceduresList) > index:
            ...
            if not index in self.procedureGUI[equipmentName]:
                self.procedureGUI[equipmentName][index] = self.procedure.GUI(
                    self.procedurePanel, equipmentName
                )
