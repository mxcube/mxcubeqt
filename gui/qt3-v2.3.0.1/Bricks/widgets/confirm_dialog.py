...
from widgets.confirm_dialog_widget_vertical_layout import (
    ConfirmDialogWidgetVerticalLayout,
)
...
class ConfirmDialog(qt.QDialog):
    ...
    def set_plate_mode(self, plate_mode):
        self.dialog_layout_widget.snapshots_list = (
            [1, 0] if plate_mode else [4, 1, 2, 0]
        )
    ...
    def set_items(self, checked_items):
        ...
        for item in checked_items:
            ...
            if path_template:
                ...
                for fp in file_paths:
                    ...
                    fl = FileListViewItem(
                        self.dialog_layout_widget.file_list_view,
                        last_item,
                        sample_name,
                        dir_name,
                        f_name,
        ...
        self.dialog_layout_widget.summary_label.setText(
            "Collecting "
            + str(num_collections)
            + " collection(s) on "
            + str(num_samples)
    ...
    def continue_button_click(self):
        for item in self.checked_items:
            ...
            if acq_params is None:
                continue
            acq_params.take_snapshots = int(
                self.dialog_layout_widget.take_snapshots_cbox.currentText()
            )
