    ...
    def propertyChanged(self, propertyName, oldValue, newValue):
        if propertyName == "icons":
            ...
            if self.sampleChanger is not None:
                ...
                self.connect(
                    self.sampleChanger,
                    PYSIGNAL("loadedSampleChanged"),
                    self.loadedSampleChanged,
                )
                self.connect(
                    self.sampleChanger, PYSIGNAL("samplesMapChanged"), self.updateVials
                )
                self.connect(
                    self.sampleChanger,
                    PYSIGNAL("matrixCodesUpdate"),
                    self.dataMatricesUpdated,
                )
                self.connect(
                    self.sampleChanger,
                    PYSIGNAL("ioStatusChanged"),
                    self.ioStatusChanged,
                )
            else:
                self.sampleChangerStateChanged("UNKNOWN")
        else:
    ...
    def updateVials(self, samples_map):
        ...
        for i in range(30):
            ...
            if samples_map[i] == "on_axis":
                self.baskets[basket].samplesView.vials[vial].setVial(
                    (SampleChangerBrick3.VialView.VIAL_AXIS, "")
                )
                self.unmount_button.setEnabled(True)
            elif samples_map[i] == "unknown":
                self.baskets[basket].samplesView.vials[vial].setVial(
                    (SampleChangerBrick3.VialView.VIAL_UNKNOWN, "")
                )
            elif samples_map[i] == "already_mounted":
                self.baskets[basket].samplesView.vials[vial].setVial(
                    (SampleChangerBrick3.VialView.VIAL_NOBARCODE_LOADED, "")
                )
