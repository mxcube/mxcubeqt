...
class CatsBrick(BaseComponents.BlissWidget):
    def __init__(self, *args):
        ...
        qt.QObject.connect(
            self.widget.btLoadSample, qt.SIGNAL("clicked()"), self._loadSample
        )
        # qt.QObject.connect(self.widget.btLoadSample,qt.SIGNAL('clicked()'),self.onMenuLoad)
        qt.QObject.connect(
            self.widget.btUnloadSample, qt.SIGNAL("clicked()"), self._unloadSample
        )
        ...
        qt.QObject.connect(
            self.list, qt.SIGNAL("selectionChanged()"), self.onListSelected
        )
        qt.QObject.connect(
            self.list,
            qt.SIGNAL("contextMenuRequested(QListViewItem*,const QPoint&,int)"),
            self.onListPopupMenu,
        )

        self.widget.ckShowEmptySlots.setChecked(False)
        qt.QObject.connect(
            self.widget.ckShowEmptySlots, qt.SIGNAL("clicked()"), self.onShowEmptySlots
        )
    ...
    def propertyChanged(self, property, oldValue, newValue):
        logging.getLogger("user_level_log").info(
            "Property Changed: " + str(property) + " = " + str(newValue)
        )
        if property == "hwobj":
            if self.device is not None:
                self.disconnect(
                    self.device, self.device.STATE_CHANGED_EVENT, self.onStateChanged
                )
                self.disconnect(
                    self.device, self.device.STATUS_CHANGED_EVENT, self.onSatusChanged
                )
                self.disconnect(
                    self.device, self.device.INFO_CHANGED_EVENT, self.onInfoChanged
                )
                self.disconnect(
                    self.device,
                    self.device.LOADED_SAMPLE_CHANGED_EVENT,
                    self.onLoadedSampleChanged,
                )
                self.disconnect(
                    self.device,
                    self.device.SELECTION_CHANGED_EVENT,
                    self.onSelectionChanged,
                )
                self.disconnect(
                    self.device, self.device.TASK_FINISHED_EVENT, self.onTaskFinished
                )
            ...
            if self.device is not None:
                ...
                self.connect(
                    self.device, self.device.STATUS_CHANGED_EVENT, self.onStatusChanged
                )
                self.connect(
                    self.device, self.device.INFO_CHANGED_EVENT, self.onInfoChanged
                )
    ...
    def onListPopupMenu(self, item, point, col):
        ...
        if col == 0 and (component is not None):
            ...
            if component == self.device:
                menu.setItemEnabled(
                    menu.insertItem("Set ID", self.onMenuSetID), component.isTransient()
                )
            menu.setItemEnabled(
                menu.insertItem("Scan", self.onMenuScan), component.isScannable()
            )
            menu.popup(point)
    ...
    def onMenuSetID(self):
        try:
            (text, ok) = qt.QInputDialog.getText(
                ...
                "Enter new  ID:",
                qt.QLineEdit.Normal,
                self._getElementID(self.device).strip(),
                self,
            )
            if ok:
            ...
            if component is not None:
                if component.isScannable():
                    ...
                    qt.QMessageBox.information(
                        self, "Scan", "Component is not scannable"
                    )
        except BaseException:
            qt.QMessageBox.warning(self, "Error", str(sys.exc_info()[1]))
    ...
    def _updateButtons(self):
        ...
        if self.device is None or not self.device.isReady():
            ...
            charging = (
                self.device.getState() == SampleChanger.SampleChangerState.Charging
            )
            ready = self.device.getState() == SampleChanger.SampleChangerState.Ready
            standby = self.device.getState() == SampleChanger.SampleChangerState.StandBy
            moving = self.device.getState() in [
                SampleChanger.SampleChangerState.Moving,
                SampleChanger.SampleChangerState.Loading,
                SampleChanger.SampleChangerState.Unloading,
            ]
            self.widget.btLoadSample.setEnabled(
                ready
                and not charging
                and (selected_sample is not None)
                and selected_sample.isPresent()
                and (selected_sample != self._loadedSample)
            )
            self.widget.btUnloadSample.setEnabled(
                ready and not charging and self.device.hasLoadedSample()
            )
            self.widget.lvSC.setEnabled(ready or standby or charging)
        self.widget.btAbort.setEnabled(
            self.device is not None and not self.device.isReady()
        )
    ...
    def _addElement(self, parent, element):
        ...
        class MyListViewItem(qt.QListViewItem):
            def paintCell(self, painter, color_group, column, width, align):
                ...
                qt.QListViewItem.paintCell(
                    self, painter, color_group, column, width, align
                )
        ...
        item = MyListViewItem(
            parent,
            element.getAddress(),
            self._getElementStatus(element),
            self._getElementID(element),
            *args
        )
