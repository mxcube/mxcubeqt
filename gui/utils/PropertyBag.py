    ...
    def set_value(self, property_value):
        ...
        for choice in self.choices:
            if strValue == choice:
                ...
                self.value = strValue
                return
        raise ValueError("%s is not a valid choice for combo" % repr(str(property_value)))
...
class FormatStringProperty(Property):
    ...
    def set_value(self, format):
        ...
        if len(parts) == 2:
            self.format_string = '%' + prefix + str(len(parts[0])) + '.' + str(len(parts[1])) + 'f'
        elif len(parts) == 1:
...
class PropertyBag:
    ...
    def add_property(self, property_name, property_type, arg1=None, arg2=None, comment="", hidden=False):
