...
class CameraBrick(BaseWidget):
    ...
    def property_changed(self, property_name, old_value, new_value):
        if property_name == "mnemonic":
            if self.graphics_manager_hwobj is not None:
                self.disconnect(
                    HWR.beamline.sample_view, "mouseMoved", self.mouse_moved
                )
                self.disconnect(
                    HWR.beamline.sample_view, "imageScaleChanged", self.image_scaled
                )
                self.disconnect(
                    HWR.beamline.sample_view, "infoMsg", self.set_info_msg
                )

            self.graphics_manager_hwobj = HWR.beamline.sample_view
            ...
            if self.graphics_manager_hwobj is not None:
                ...
                self.connect(HWR.beamline.sample_view, "infoMsg", self.set_info_msg)
                self.graphics_view = HWR.beamline.sample_view.get_graphics_view()
                # self.graphics_camera_frame = HWR.beamline.sample_view.get_camera_frame()
                self.main_layout.addWidget(self.graphics_view)
                self.main_layout.addWidget(self.info_widget)
                self.set_fixed_size()
                self.init_image_scale_list()
                if hasattr(HWR.beamline.sample_view, "camera"):
    ...
    def set_control_mode(self, have_control):
        if have_control:
            HWR.beamline.sample_view.hide_info_msg()
        else:
            HWR.beamline.sample_view.display_info_msg(
                [
                    "",
    ...
    def set_fixed_size(self):
        if self.fixed_size and HWR.beamline.sample_view:
            HWR.beamline.sample_view.set_graphics_scene_size(self.fixed_size, True)
    ...
    def display_histogram_toggled(self):
        HWR.beamline.sample_view.display_histogram(
    ...
    def create_point_current_clicked(self):
        HWR.beamline.sample_view.start_centring(tree_click=False)
    ...
    def zoom_window_clicked(self):
        self.zoom_dialog.set_camera_frame(
            HWR.beamline.sample_view.get_camera_frame()
        )
    ...
    def display_radiation_damage_toggled(self):
        HWR.beamline.sample_view.display_radiation_damage(
...
class CameraControlDialog(QtImport.QDialog):
    ...
    def set_camera_hwobj(self, camera_hwobj):
        ...
        # get attribute value
        try:
            contrast_value = HWR.beamline.sample_view.camera.get_contrast()
        except AttributeError:
            contrast_value = None
        try:
            brightness_value = HWR.beamline.sample_view.camera.get_brightness()
        except AttributeError:
            brightness_value = None
        try:
            gain_value = HWR.beamline.sample_view.camera.get_gain()
        except AttributeError:
            gain_value = None
        try:
            gamma_value = HWR.beamline.sample_view.camera.get_gamma()
        except AttributeError:
            gamma_value = None
        try:
            exposure_time_value = HWR.beamline.sample_view.camera.get_exposure_time()
        except AttributeError:
            exposure_time_value = None

        # get attribute auto state
        try:
            contrast_auto = HWR.beamline.sample_view.camera.get_contrast_auto()
        except AttributeError:
            contrast_auto = None
        try:
            brightness_auto = HWR.beamline.sample_view.camera.get_brightness_auto()
        except AttributeError:
            brightness_auto = None
        try:
            gain_auto = HWR.beamline.sample_view.camera.get_gain_auto()
        except AttributeError:
            gain_auto = None
        try:
            gamma_auto = HWR.beamline.sample_view.camera.get_gamma_auto()
        except AttributeError:
            gamma_auto = None
        try:
            exposure_time_auto = HWR.beamline.sample_view.camera.get_exposure_time_auto()
        except AttributeError:
            exposure_time_auto = None

        # get attribute range
        try:
            contrast_min_max = HWR.beamline.sample_view.camera.get_contrast_min_max()
        except AttributeError:
            contrast_min_max = (0, 100)
        try:
            brightness_min_max = HWR.beamline.sample_view.camera.get_brightness_min_max()
        except AttributeError:
            brightness_min_max = (0, 100)
        try:
            gain_min_max = HWR.beamline.sample_view.camera.get_gain_min_max()
        except AttributeError:
            gain_min_max = (0, 100)
        try:
            gamma_min_max = HWR.beamline.sample_view.camera.get_gamma_min_max()
        except AttributeError:
            gamma_min_max = (0, 100)
        try:
            exposure_time_min_max = HWR.beamline.sample_view.camera.get_exposure_time_min_max()
        except AttributeError:
    ...
    def set_contrast(self, value):
        ...
        HWR.beamline.sample_view.camera.set_contrast(value)

    def set_brightness(self, value):
        self.brightness_slider.setValue(value)
        self.brightness_doublespinbox.setValue(value)
        HWR.beamline.sample_view.camera.set_brightness(value)

    def set_gain(self, value):
        self.gain_slider.setValue(value)
        self.gain_doublespinbox.setValue(value)
        HWR.beamline.sample_view.camera.set_gain(value)

    def set_gamma(self, value):
        self.gamma_slider.setValue(value)
        self.gamma_doublespinbox.setValue(value)
        HWR.beamline.sample_view.camera.set_gamma(value)

    def set_exposure_time(self, value):
        self.exposure_time_slider.setValue(value)
        self.exposure_time_doublespinbox.setValue(value)
        HWR.beamline.sample_view.camera.set_exposure_time(value)

    def set_contrast_auto(self, state):
        state = bool(state)
        HWR.beamline.sample_view.camera.set_contrast_auto(state)
        value = HWR.beamline.sample_view.camera.get_contrast()
        self.contrast_slider.setValue(value)
        self.contrast_doublespinbox.setValue(value)

    def set_brightness_auto(self, state):
        state = bool(state)
        HWR.beamline.sample_view.camera.set_brightness_auto(state)
        value = HWR.beamline.sample_view.camera.get_brightness()
        self.brightness_slider.setValue(value)
        self.brightness_doublespinbox.setValue(value)

    def set_gain_auto(self, state):
        state = bool(state)
        HWR.beamline.sample_view.camera.set_gain_auto(state)
        value = HWR.beamline.sample_view.camera.get_gain()
        self.gain_slider.setValue(value)
        self.gain_doublespinbox.setValue(value)

    def set_gamma_auto(self, state):
        state = bool(state)
        HWR.beamline.sample_view.camera.set_gamma_auto(state)
        value = HWR.beamline.sample_view.camera.get_gamma()
        self.gamma_slider.setValue(value)
        self.gamma_doublespinbox.setValue(value)

    def set_exposure_time_auto(self, state):
        state = bool(state)
        HWR.beamline.sample_view.camera.set_exposure_time_auto(state)
        value = HWR.beamline.sample_view.camera.get_exposure_time()
        self.exposure_time_slider.setValue(value)
        self.exposure_time_doublespinbox.setValue(value)
