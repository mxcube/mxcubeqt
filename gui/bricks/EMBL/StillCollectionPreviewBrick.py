...
class StillCollectionPreviewBrick(BaseWidget):
    ...
    def __init__(self, *args):
        ...
        self.grid_cell_label = QtImport.QLabel("Selected grid cell: A1", self.grid_widget)
        ...
        self.save_hit_plot_button = QtImport.QPushButton("Full grid view", self.grid_widget)

        self.comp_widget = QtImport.QWidget(self)
        self.comp_cell_label = QtImport.QLabel("Selected compartment cell: A1", self.comp_widget)
    ...
    def property_changed(self, property_name, old_value, new_value):
        if property_name == 'hwobj_beamline_setup':
            if self.parallel_processing_hwobj is not None:
                self.parallel_processing_hwobj.disconnect("processingStarted",
            ...
            if self.parallel_processing_hwobj is not None:
                self.parallel_processing_hwobj.connect("processingStarted",
                                                       self.processing_started)
    ...
    def init_gui(self):
        ...
        table_height = self['cell_size'] * (self.grid_properties["grid_num_row"] + 1) + 4
        ...
        table_height = self['cell_size'] * (self.grid_properties["comp_num_row"] + 1) + 7
    ...
    def save_grid_view(self):
        ...
        filename += ".png"
        image = QtImport.QImage(self.grid_graphics_view.scene().sceneRect().\
            size().toSize(), QtImport.QImage.Format_ARGB32)
    ...
    def grid_cell_entered(self, row, col):
        if not self.grid_table_item_fixed:
            ...
            if self.params_dict:
                ...
                filename = self.params_dict['template'] % (self.params_dict['run_number'], index)
                try:
    ...
    def update_gui(self):
        ...
        self.info_dict["comp_num"] = self.frame_num / self.params_dict["num_images_per_trigger"]
        ...
        comp_cell = (self.frame_num - grid_cell * \
                     self.grid_properties["comp_num_col"] * \
                     self.grid_properties["comp_num_row"] * \
                     self.params_dict["num_images_per_trigger"]) / \
                     self.params_dict["num_images_per_trigger"]
...
class GridViewGraphicsItem(QtImport.QGraphicsItem):
                ...
                for compartment_x in range(self.no_compartments_x):
                    for hole_y in range(1, self.no_holes_y + 1):
                        for hole_x in range(1, self.no_holes_x + 1):
                            ...
                            corner_x = (compartment_x * \
                                        (self.size_compartment_x + self.offset_compartment)) + \
                                        (hole_x * (self.size_hole + self.offset_hole))
                            corner_y = (compartment_y * (self.size_compartment_y + \
                                       self.offset_compartment)) + \
    ...
    def init_item(self, params_dict, results=None):
        ...
        self.size_hole = 2000 / (self.no_compartments_x * \
...
class GridViewOverlayItem(GridViewGraphicsItem):
    ...
    def calc_hole_coordinates(self, imgno):
        ...
        return compartments_serial, holes_serial, timepoint_serial, \
              compartment_x, compartment_y, hole_x, hole_y, \
              timepoint_x, timepoint_y

    def paint(self, painter, option, widget):
        self.custom_brush.setColor(QtImport.Qt.blue)
        painter.setBrush(self.custom_brush)
        def draw_hole_timeseries(compartment_x, compartment_y, hole_x,\
                                 hole_y, subpixel_x, subpixel_y):
            #compartment_y = self.no_compartments_y - compartment_y +1
            #hole_y        = self.no_holes_y - hole_y +1
        ...
        if self.results is not None:
            ...
            for hit in hitlist:
                compartments_serial, holes_serial, \
                timepoint_serial, compartment_x, \
                compartment_y, hole_x, hole_y, \
                timepoint_x, timepoint_y = self.calc_hole_coordinates(hit)
                if timepoint_y <= 1:
                     draw_hole_timeseries(compartment_x, compartment_y,
                                hole_x, hole_y, timepoint_x, timepoint_y)
