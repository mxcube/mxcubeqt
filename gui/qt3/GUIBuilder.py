...
class ToolboxWindow(qt.QWidget):
    ...
    def brickTextLabel(self, brickName):
        ...
        for i in range(len(brickTextLabel)):
            if brickTextLabel[i].isalpha() or brickTextLabel[i].isdigit():
                if i > 0 and brickTextLabel[i].isupper(
                ) and brickTextLabel[i - 1].islower():
                    brickTextLabel = brickTextLabel[0:i] + " " + \
                        brickTextLabel[i].lower() + brickTextLabel[i + 1:]
            else:
    ...
    def addBricks(self, brickDir):
        ...
        findCategoryRE = re.compile(r"^__category__\s*=\s*['\"](.*)['\"]$", re.M)
        ...
        for fileordir in os.listdir(brickDir):
            ...
            if os.path.isdir(full_path):
                ...
                filesNames = [os.path.join(full_path, filename)
        ...
        for full_filename in full_filenames:
            ...
            if filter(lambda x: filename.endswith(x), [
                      x[0] for x in imp.get_suffixes()]):
                ...
                if not brickName == '__init__' and not brickName in processedBricks:
                    ...
                    try:
                        brick_module_file, path, description = imp.find_module(brickName, [
                                                                               dirname])
                    ...
                    checkIfItsBrick = re.compile(
                        r'^\s*class\s+%s.+?:\s*$' %
                        brickName, re.M)
                    if not checkIfItsBrick.search(moduleContents):
                        continue
                    ...
                    try:
                        bricksCategories[category].append(
                            (brickName, dirname, description))
                    except KeyError:
        ...
        categoryKeys = sorted(bricksCategories.keys())
        ...
        for category in categoryKeys:
            ...
            for brickName, dirname, description in bricksList:
                newBrick = qt.QListViewItem(
                    bricksListWidget, self.brickTextLabel(brickName))
...
class GUIEditorWindow(qt.QWidget):
    def __init__(self, *args, **kwargs):
        ...
        qt.QObject.connect(
            self.treeLayout,
            qt.SIGNAL('itemRenamed(QListViewItem *, int)'),
            self.itemRenamed)
        qt.QObject.connect(
            self.treeLayout,
            qt.PYSIGNAL('dragdrop'),
            self.itemDragDropped)
        qt.QObject.connect(
            self.treeLayout,
            qt.PYSIGNAL('contextMenuRequested'),
    ...
    def setConfiguration(self, configuration):
        ...
        def addChildren(children, parentItem):
            ...
            for child in children:
                ...
                if self.configuration.isContainer(child):
                    #print 'is container'
                    new_list_item = self.appendItem(
                        parentItem, child["name"], child["type"], icon=child["type"])
                    ...
                    #print 'is spacer'
                    new_list_item = self.appendItem(
                        parentItem, child["name"], "spacer", icon=child["type"])
                elif self.configuration.isBrick(child):
                    #print 'is brick'
                    new_list_item = self.appendItem(
                        parentItem, child["name"], child["type"], icon="brick")
                else:
        ...
        for window in self.configuration.windows_list:
            parentWindowItem = self.appendItem(
                self.rootElement, window.name, "window", icon="window_small")
        ...
        if currentItem:
            ...
            if children > 0:
                if qt.QMessageBox.warning(self, "Please confirm",
                                          "Are you sure you want to remove %s ?\nRemoving item will remove its %d children." % (
                                              item_name, children),
    ...
    def connectItem(self, parent, new_item, new_list_item):
        if self.configuration.isBrick(new_item):
            ...
            def property_changed_cb(property, old_value, new_value, itemref=weakref.ref(
                    new_list_item), parentref=parentref):
                item = itemref()
                if item is None:
                    #print 'NONE !'
                    return
                ...
                if parentref is not None:
                    ...
                    parent.childPropertyChanged(
                        item_name, property, old_value, new_value)
    ...
    def _addItem(self, parentListItem, item_type, *args):
        ...
        try:
            ...
            if item_type == "window":
                ...
                if isinstance(new_item, types.StringType):
                    qt.QMessageBox.warning(
                        self, "Cannot add", new_item, qt.QMessageBox.Ok)
                else:
                    new_item["properties"].getProperty(
                        "w").setValue(qt.qApp.desktop().width())
                    new_item["properties"].getProperty(
                        "h").setValue(qt.qApp.desktop().height())

                    new_list_item = self.appendItem(
                        parentListItem, new_item["name"], "window", icon="window_small")
                ...
                if isinstance(new_item, types.StringType):
                    ...
                    new_list_item = self.appendItem(
                        parentListItem, brick_name, brick_type, icon="brick")
            elif item_type == "tab":
                ...
                if isinstance(new_item, types.StringType):
                    ...
                    new_list_item = self.appendItem(
                        parentListItem, item_name, item_type, icon=item_type)
            else:
                item_subtype = args[0]
                ...
                if isinstance(new_item, types.StringType):
                    ...
                    new_list_item = self.appendItem(
                        parentListItem, item_name, item_type, icon=item_subtype)

            if not isinstance(new_item, types.StringType) and new_item is not None:
    ...
    def itemRightClicked(self, item):
        ...
        if self.configuration.isBrick(item_cfg):
            ...
            def _itemRightClicked(id, item_name=item_name):
                ...
                self.emit(
                    qt.PYSIGNAL("newItem"),
                    (item_cfg["brick"].propertyBag,
                     item_cfg["brick"]._propertyChanged))
                #print 'itemSelected'
...
class GUIPreviewWindow(qt.QWidget):
    def __init__(self, *args, **kwargs):
        ...
        qt.QObject.connect(
            self.windowPreview,
            qt.PYSIGNAL("itemClicked"),
            self.previewItemClicked)
    ...
    def drawWindow(self, container_cfg, window_id, container_ids, selected_item):
        ...
        if container_cfg["type"] == "window":
            ...
            self.windowPreviewBox.setTitle(
                "Window preview: %s%s" %
                (container_cfg["name"], s))

        self.windowPreview.updatePreview(
...
class GUIBuilder(qt.QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.cmdLaunchGUI = ToolButton(
            toolbar,
            "launch",
            self.launchGUIClicked,
            "launch GUI (as a separate process)")
        HorizontalSpacer(toolbar, size=20)
        self.cmdViewPropertyEditor = ToolButton(
            toolbar, "Draw", self.showProperties, "show properties window")
        self.cmdViewGuiPreview = ToolButton(
            toolbar,
            "window_preview",
            self.showGuiPreview,
            "show GUI preview")
        self.cmdViewLog = ToolButton(
            toolbar,
            "Inform",
            self.showLog,
            "show log messages window")
        self.cmdViewHWR = ToolButton(
            toolbar,
            "view_tree",
            self.showHWR,
            "show Hardware Repository")
        ...
        qt.QObject.connect(
            self.toolboxWindow,
            qt.PYSIGNAL("addBrick"),
            self.guiEditorWindow.addBrick)
        qt.QObject.connect(
            self.guiEditorWindow,
            qt.PYSIGNAL("editProperties"),
            self.propertyEditorWindow.editProperties)
        qt.QObject.connect(
            self.guiEditorWindow,
            qt.PYSIGNAL("newItem"),
            self.propertyEditorWindow.addProperties)
        qt.QObject.connect(
            self.guiEditorWindow,
            qt.PYSIGNAL("updatePreview"),
            self.guiPreviewWindow.drawWindow)
        qt.QObject.connect(
            self.guiPreviewWindow,
            qt.PYSIGNAL("previewItemClicked"),
            self.guiEditorWindow.selectItem)
        qt.QObject.connect(
            self.guiEditorWindow,
            qt.PYSIGNAL("showProperties"),
            self.showProperties)
        qt.QObject.connect(
            self.guiEditorWindow,
            qt.PYSIGNAL("hideProperties"),
            self.hideProperties)
        qt.QObject.connect(
            self.guiEditorWindow,
            qt.PYSIGNAL("showPreview"),
            self.showGuiPreview)
    ...
    def openClicked(self):
        ...
        if len(filename) > 0:
            try:
                ...
                qt.QMessageBox.warning(
                    self, "Error", "Could not open file %s !" %
                    filename, qt.QMessageBox.Ok)
            else:
                try:
                    ...
                    try:
                        ...
                        qt.QMessageBox.warning(
                            self, "Error", "Could not read configuration\nfrom file %s" %
                            filename, qt.QMessageBox.Ok)
                    else:
                        self.filename = filename
            ...
            if self.filename is not None:
                ...
                if self.configuration.save(self.filename):
                    ...
                    qt.QMessageBox.information(
                        self, "Success", "Configuration have been saved successfully to\n%s" %
                        self.filename, qt.QMessageBox.Ok)
                    ...
                    qt.qApp.restoreOverrideCursor()
                    qt.QMessageBox.warning(
                        self, "Error", "Could not save configuration to file %s !" %
                        self.filename, qt.QMessageBox.Ok)
    ...
    def launchGUIClicked(self):
        ...
        try:
            ...
            customBricksDirs = os.path.pathsep.join(
                BlissFramework.getCustomBricksDirs())
            pid = subprocess.Popen(
                "%s -title %s -e startGUI --bricksDirs=%s %s%s" %
                (terminal, os.path.basename(
                    self.filename), customBricksDirs, (hwr_server and "--hardwareRepository=%s " %
                                                       hwr_server or ""), self.filename), shell=True).pid
