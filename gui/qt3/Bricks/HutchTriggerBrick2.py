    ...
    def propertyChanged(self, propertyName, oldValue, newValue):
        if propertyName == "mnemonic":
            if self.hutchTrigger is not None:
                self.disconnect(
                    self.hutchTrigger, PYSIGNAL("connected"), self.connected
                )
                self.disconnect(
                    self.hutchTrigger, PYSIGNAL("disconnected"), self.connected
                )
                self.disconnect(
                    self.hutchTrigger, PYSIGNAL("statusChanged"), self.statusChanged
                )
                self.disconnect(
                    self.hutchTrigger, PYSIGNAL("msgChanged"), self.msgChanged
                )
                self.disconnect(
                    self.hutchTrigger,
                    PYSIGNAL("macroStarted"),
                    self.hutchTriggerStarted,
                )
                self.disconnect(
                    self.hutchTrigger, PYSIGNAL("macroDone"), self.hutchTriggerDone
                )
                self.disconnect(
                    self.hutchTrigger, PYSIGNAL("macroFailed"), self.hutchTriggerFailed
                )
                self.disconnect(
                    self.hutchTrigger,
