        ...
        if prop == "xmlFile":
            ...
            if not self.isRunning():
                for channel in self.hwo.getChannels():
                    self.addProperty(
                        "%s expert only" % channel.name(), "boolean", False
                    )
        elif prop == "uiFile":
            ...
            if prop.endswith("expert only"):
                ...
                self.getProperty("expert_channels").setValue(
                    str(self.property_expert_channels)
                )
        ...
        for checkbox in all_checkboxes:
            ...
            try:
                ...
                logging.getLogger().exception(
                    "%s: could not set expert property for channel %s",
                    self.name(),
                    channel_name,
                )
