...
class CharacteriseSimpleWidgetVerticalLayout(QWidget):
    def __init__(self, parent=None, name=None, fl=0):
        ...
        CharacteriseSimpleWidgetVerticalLayoutLayout = QHBoxLayout(
            self, 0, 0, "CharacteriseSimpleWidgetVerticalLayoutLayout"
        )

        main_vlayout = QVBoxLayout(None, 0, 6, "main_vlayout")

        self.characterisation_gbox = QGroupBox(self, "characterisation_gbox")
        self.characterisation_gbox.setSizePolicy(
            QSizePolicy(
                QSizePolicy.Expanding,
                QSizePolicy.Expanding,
                0,
                0,
                self.characterisation_gbox.sizePolicy().hasHeightForWidth(),
            )
        )
        ...
        self.start_comp_cbox = QComboBox(
            0, self.characterisation_gbox, "start_comp_cbox"
        )
        ...
        self.account_rad_dmg_cbx = QCheckBox(
            self.characterisation_gbox, "account_rad_dmg_cbx"
        )
        checkbox_layout.addWidget(self.account_rad_dmg_cbx)

        self.optimised_sad_cbx = QCheckBox(
            self.characterisation_gbox, "optimised_sad_cbx"
        )
        checkbox_layout.addWidget(self.optimised_sad_cbx)

        self.induced_burn_cbx = QCheckBox(
            self.characterisation_gbox, "induced_burn_cbx"
        )
