...
class CreateTaskBase(qt.QWidget):
        ...
        if bl_setup is not None:
            if self._acq_widget:
                ...
                self._acquisition_parameters = (
                    bl_setup.get_default_acquisition_parameters()
                )
        else:
    ...
    def set_beamline_setup(self, bl_setup_hwobj):
        ...
        try:
            ...
            if bl_setup_hwobj.kappa_axis_hwobj is not None:
                bl_setup_hwobj.kappa_axis_hwobj.connect(
            ...
            if (not free_pin_mode) and (not sample_is_mounted) or (not shape):
                ...
                if temp_tasks[0].requires_centring():
                    ...
                    if isinstance(temp_tasks[0], queue_model_objects.DataCollection):
                        kappa = (
                            temp_tasks[0].acquisitions[0].acquisition_parameters.kappa
                        )
                        kappa_phi = (
                            temp_tasks[0]
                            .acquisitions[0]
                            .acquisition_parameters.kappa_phi
                        )
                        if kappa and kappa_phi:
                            task_label = (
                                "sample-centring (kappa: %0.2f, phi: %0.2f)"
                                % (kappa, kappa_phi)
                            )
                    elif isinstance(
    ...
    def _create_acq(self, sample):
        ...
        acq.acquisition_parameters = copy.deepcopy(parameters)
