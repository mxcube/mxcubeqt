...
class DataModelInputBinder(object):
    ...
    def __validated(self, validator, widget, new_value):
        if validator:
            if (
                validator.validate(new_value, widget.cursorPosition())[0]
    ...
    def _update_widget(self, field_name, data_binder):
        ...
        try:
            ...
            elif isinstance(widget, qt.QCheckBox) or isinstance(
    ...
    def bind_value_update(self, field_name, widget, type_fn, validator=None):
        ...
        if isinstance(widget, qt.QLineEdit):
            qt.QObject.connect(
                ...
                lambda new_value: self.__ledit_update_value(
                    field_name, new_value, type_fn, validator
                ),
            )
            if type_fn is float and validator:
                pattern = "%." + str(validator.decimals()) + "f"
        ...
        elif isinstance(widget, qt.QCheckBox) or isinstance(widget, qt.QRadioButton):
            qt.QObject.connect(
                widget,
    ...
    def validate_all(self):
        ...
        for (key, value) in self.bindings.iteritems():
            ...
            if validator:
                if isinstance(widget, qt.QLineEdit):
                    if not self.__validated(validator, widget, widget.text()):
                        result.append(key)
                elif isinstance(widget, qt.QComboBox):
                    pass
                elif isinstance(widget, qt.QCheckBox) or isinstance(
