...
class ScanDimensionBox(QHBox):
    def __init__(self, parent, minDim=1, maxDim=3):
        ...
        for i in range(self.minDim, self.maxDim + 1):
            ...
            self.connect(self.dimensions[-1],
                         SIGNAL('clicked()'),
    ...
    def dimensionChanged(self):
        for dim in self.dimensions:
            if dim == self.sender():
                self.emit(PYSIGNAL('dimensionChanged'),
                          (self.dimensions.index(dim) + 1, ))
                return
...
class MotorPositionReminder(QTableItem):
    ...
    def createEditor(self):
        self.cmdMotorPosition = QPushButton(
            MotorPositionReminder.NOPOSITION,
            self.table().viewport())
    ...
    def motorPositionChanged(self, position):
        if not str(position).isalpha():
            ...
            self.cmdMotorPosition.setEnabled(False)
            self.cmdMotorPosition.setText(str(position).center(
                len(MotorPositionReminder.NOPOSITION)))
...
class AxisConfigurationTable(QTable):
    ...
    def sizeHint(self):
        return QSize(self.contentsWidth() + self.leftMargin() + 2 * self.frameWidth(),
...
class GenericScanBrick(BaseComponents.ProcedureBrick):
    def __init__(self, *args):
        ...
        #
        self.connect(
            self.scanDimensionBox,
            PYSIGNAL('dimensionChanged'),
            self.dimensionChanged)
        self.connect(
