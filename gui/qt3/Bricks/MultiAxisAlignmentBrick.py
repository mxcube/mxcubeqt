    ...
    def close(self):
        if self.__verticalMotor is not None:
            self.__brick.disconnect(
                self.__verticalMotor,
                qt.PYSIGNAL("stateChanged"),
                self.__updateVMotorState,
            )
            self.__brick.disconnect(
                self.__verticalMotor,
                qt.PYSIGNAL("positionChanged"),
                self.__updateVMotorPosition,
            )
        if self.__horizontalMotor is not None:
            self.__brick.disconnect(
                self.__horizontalMotor,
                qt.PYSIGNAL("stateChanged"),
                self.__updateHMotorState,
            )
            self.__brick.disconnect(
                self.__horizontalMotor,
                qt.PYSIGNAL("positionChanged"),
                self.__updateHMotorPosition,
            )
        if self.__rotationMotor is not None:
            self.__brick.disconnect(
                self.__rotationMotor,
                qt.PYSIGNAL("stateChanged"),
                self.__updateHMotorState,
            )
            self.__brick.disconnect(
    ...
    def init(self):
        ...
        if self.__rotationMotor is not None:
            ...
            self.__brick.connect(
                self.__rotationMotor,
                qt.PYSIGNAL("stateChanged"),
                self.__updateRMotorState,
            )
            self.__brick.connect(
                self.__rotationMotor,
                qt.PYSIGNAL("positionChanged"),
                self.__updateRMotorPosition,
            )
...
class MultiAxisAlignmentBrick(BlissWidget):
    def __init__(self, *args):
        ...
        self.addProperty(
            "dynamicProperties", "", PropertyBag.PropertyBag(), hidden=True
        )
        ...
        if property.find("steps") > -1:
            ...
            try:
                ...
                logging.getLogger().error(
                    "steps must be a float list separate by space"
                )
    ...
    def __addNewProperty(self, equipment, padid=0, addNumberFlag=False):
        ...
        try:
            if equipment.getDeviceByRole("horizontal") is not None:
                ...
                if hasattr(equipment, "isMultiAxis"):
                    stringsteps = " ".join(
                        [str(x) for x in equipment.getStepsByRole("horizontal")]
                    )
                    defaultArrowDirection = equipment.getArrowMotorMoveByRole(
                        "horizontal"
                    )
                self["dynamicProperties"].addProperty(
                    "%s%s" % (padstring, "hori. steps"), "string", stringsteps
                )
                self["dynamicProperties"].addProperty(
                    "%s%s" % (padstring, "hori. arrow/motormove"),
                    "combo",
                    ("positive2right", "negative2right"),
        ...
        try:
            if equipment.getDeviceByRole("vertical") is not None:
                ...
                if hasattr(equipment, "isMultiAxis"):
                    stringsteps = " ".join(
                        [str(x) for x in equipment.getStepsByRole("vertical")]
                    )
                    defaultArrowDirection = equipment.getArrowMotorMoveByRole(
                        "vertical"
                    )
                self["dynamicProperties"].addProperty(
                    "%s%s" % (padstring, "vert. steps"), "string", stringsteps
                )
                self["dynamicProperties"].addProperty(
                    "%s%s" % (padstring, "vert. arrow/motormove"),
                    "combo",
                    ("positive2up", "negative2up"),
                    defaultArrowDirection,
                )
        except BaseException:
            pass
        try:
            if equipment.getDeviceByRole("rotation") is not None:
                ...
                if hasattr(equipment, "isMultiAxis"):
                    stringsteps = " ".join(
                        [str(x) for x in equipment.getStepsByRole("rotation")]
                    )
                    defaultArrowDirection = equipment.getArrowMotorMoveByRole(
                        "rotation"
                    )
                self["dynamicProperties"].addProperty(
                    "%s%s" % (padstring, "rot. steps"), "string", stringsteps
                )
                self["dynamicProperties"].addProperty(
                    "%s%s" % (padstring, "rot. arrow/motormove"),
                    "combo",
        ...
        try:
            self["dynamicProperties"].addProperty(
                "%s%s" % (padstring, "init percent size"), "float", 1.0
            )
        except BaseException:
            import traceback
    ...
    def __addNewAxisProperty(self):
        padstring = "pad 0 :"
        self["dynamicProperties"].addProperty(
            "%s%s" % (padstring, "hori. steps"), "string", "1."
        )
        self["dynamicProperties"].addProperty(
            "%s%s" % (padstring, "init percent size"), "float", 1.0
        )
