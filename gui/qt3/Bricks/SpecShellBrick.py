...
class SpecShellBrick(BaseComponents.BlissWidget):
    ...
    def __init__(self, *args):
        ...
        QObject.connect(
            self.executeButton, SIGNAL("clicked()"), self.executeSpecCommand
        )
    ...
    def setExpertMode(self, state):
        ...
        if self.commandEditor is not None:
            QObject.disconnect(
                self.commandEditor, SIGNAL("returnPressed()"), self.executeSpecCommand
            )
            self.commandEditor = None
    ...
    def propertyChanged(self, propertyName, oldValue, newValue):
        if propertyName == "mnemonic":
            if self.specShellHO is not None:
                ...
                self.disconnect(
                    self.specShellHO, "allCommandsList", self.allCommandsList
                )
            ...
            try:
                ...
                logging.getLogger().exception(
                    "SpecShellBrick: problem setting background color"
                )
            else:
            ...
            try:
                ...
                logging.getLogger().exception(
                    "SpecShellBrick: problem setting text color"
                )
            else:
