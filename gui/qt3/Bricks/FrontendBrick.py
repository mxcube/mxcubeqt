...
class FrontendBrick(SynopticBrick.SynopticBrick):
    ...
    def __init__(self, *args):
        ...
        #
        self.connect(
            self.cmdOpenShutter, SIGNAL("clicked()"), self.cmdOpenShutterClicked
        )
        self.connect(
            self.cmdCloseShutter, SIGNAL("clicked()"), self.cmdCloseShutterClicked
        )
    ...
    def updateGUI(self):
        ...
        if self.shutter is not None:
            self.shutterStateChanged(
                self.shutter.getShutterState(), self.shutter.getAutomaticModeTimeLeft()
            )

            self.connect(
                self.shutter, PYSIGNAL("shutterStateChanged"), self.shutterStateChanged
            )
        else:
            self.cmdOpenShutter.setEnabled(False)
            self.cmdCloseShutter.setEnabled(False)
    ...
    def shutterStateChanged(self, state, automaticModeTimeLeft):
        ...
        self.lblShutter.setPaletteBackgroundColor(
            QColor(FrontendBrick.shutterState[state])
        )
        if state == "running":
            state = "automatic"
        if state == "opened" or state == "automatic":
            if state == "automatic":
                try:
                    self.lblShutter.setText(
                        str(self.lblShutter.text())
                        + "<br><nobr>%dh%dm left</nobr>"
                        % (
                            int(automaticModeTimeLeft),
                            int(automaticModeTimeLeft * 60) % 60,
                        )
                    )
    ...
    def propertyChanged(self, propertyName, oldValue, newValue):
        if propertyName == "mnemonic":
            ...
            SynopticBrick.SynopticBrick.propertyChanged.im_func(
