...
class BeamstopBrick(DuoStateBrick):
    ...
    def __init__(self, *args):
        ...
        self.alignmentDialog = BeamStopAlignmentDialog(
            "Beamstop alignment", self.beamstopAligned, self.alignDismissClicked
        )
        self.beamstop = None
        self.positions = {}
    ...
    def propertyChanged(self, propertyName, oldValue, newValue):
        if propertyName == "mnemonic":
            if self.beamstop is not None:
                self.disconnect(
                    self.beamstop, PYSIGNAL("equipmentReady"), self.equipmentReady
                )
                self.disconnect(
                    self.beamstop, PYSIGNAL("equipmentNotReady"), self.equipmentNotReady
                )
                self.disconnect(
                    self.beamstop, PYSIGNAL("positionReached"), self.positionChanged
                )
                self.disconnect(
                    self.beamstop, PYSIGNAL("noPosition"), self.positionChanged
                )
            ...
            if self.beamstop is not None:
                ...
                if (
                    not "in" in self.beamstop.positions
                    or not "out" in self.beamstop.positions
                ):
                    logging.getLogger().warning(
                        "%s: vertical beamstop motor does not define In and Out positions",
                        str(self.name()),
                    )
                ...
                # do some checkings...
                try:
                    bstopz = self.beamstop.getDeviceByRole(
                        "vertical"
                    ).getMotorMnemonic()
                    bstopy = self.beamstop.getDeviceByRole(
                        "horizontal"
                    ).getMotorMnemonic()
                ...
                if self.beamstop.isReady():
                    self.beamstop.setNewPositions(
                        "out",
                        {
                            "vertical": self.beamstop.getDeviceByRole(
                                "vertical"
                            ).getLimits()[0]
                        },
                    )
            ...
            DuoStateBrick.propertyChanged.im_func(
    ...
    def setOut(self, state):
        ...
        self.beamstop.setNewPositions(
            "out",
            {"vertical": self.beamstop.getDeviceByRole("vertical").getLimits()[0]},
        )
        if state:
    ...
    def beamstopAligned(self):
        try:
            ...
            newOutPos = newInPos
            newOutPos["vertical"] = self.beamstop.getDeviceByRole(
                "vertical"
            ).getLimits()[0]
            ...
            QMessageBox.information(
                self,
                "Beamstop alignment",
                "New beamstop positions saved successfully.",
                QMessageBox.Ok,
            )
            return
        ...
        QMessageBox.warning(
            self,
            "Beamstop alignment",
            "An error occured while trying to save beamstop positions.",
            QMessageBox.Cancel,
        )
