...
class ZapScanBrick(BaseComponents.BlissWidget):
    def __init__(self, *args):
        ...
        self.fastAxisConfiguration = GenericScanBrick.AxisConfigurationTable(
            axisConfigurationBox
        )
        self.axisConfiguration = GenericScanBrick.AxisConfigurationTable(
            axisConfigurationBox
        )
        ...
        #
        self.connect(
            self.axisConfiguration, PYSIGNAL("motorSelected"), self.motorSelected
        )
        self.connect(
            self.scanDimensionBox, PYSIGNAL("dimensionChanged"), self.dimensionChanged
        )
        self.connect(
            self.countTimeBox, PYSIGNAL("countTimeChanged"), self.countTimeChanged
        )
        self.connect(
            self.axisConfiguration, PYSIGNAL("nbPointsChanged"), self.nbPointsChanged
        )
        self.connect(
            self.fastAxisConfiguration,
            PYSIGNAL("motorSelected"),
            self.fastAxisMotorSelected,
        )
        self.connect(
            self.fastAxisConfiguration,
            PYSIGNAL("nbPointsChanged"),
            self.nbPointsChanged,
        )
    ...
    def launchScan(self):
        if self.scan is not None:
            ...
            self.scanConfig[
                "fastAxis"
            ] = self.fastAxisConfiguration.getScanConfiguration()
