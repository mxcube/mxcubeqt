...
class MotorSpinBoxBrick(BaseComponents.BlissWidget):
    ...
    def __init__(self, *args):
        ...
        QToolTip.add(
            self.spinBox,
            "Moves the motor to a specific position or step by step; right-click for motor history",
        )
        ...
        self.connect(
            self.spinBox.editor(), SIGNAL("returnPressed()"), self.valueChangedStr
        )
    ...
    def openHistoryMenu(self):
        ...
        menu.insertItem(
            QLabel("<nobr><b>%s history</b></nobr>" % self.motor.userName(), menu)
        )
    ...
    def openStepEditor(self):
        if self.isRunning():
            ...
            self.stepEditor.setMotor(
                self.motor, self, self["label"], self["defaultStep"]
            )
    ...
    def setMotor(self, motor, motor_ho_name=None):
        if self.motor is not None:
            ...
            self.disconnect(
                self.motor, PYSIGNAL("positionChanged"), self.positionChanged
            )
    ...
    def propertyChanged(self, propertyName, oldValue, newValue):
        if propertyName == "mnemonic":
            ...
            if newValue == "":
                QToolTip.add(
                    self.moveLeftButton, "Moves the motor down (while pressed)"
                )
            else:
...
class mySpinBox(QSpinBox):
    ...
    def __init__(self, parent):
        ...
        QObject.connect(
            self.editor(), SIGNAL("textChanged(const QString &)"), self.textChanged
        )
        self.rangeChange()
        self.updateDisplay()
