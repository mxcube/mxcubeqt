...
class SynopticBrick(BlissWidget):
    def __init__(self, *args):
        ...
        self.__topPanel.layout().addWidget(
            self.__lblTitle, 0, Qt.AlignTop | Qt.AlignHCenter
        )
        self.__topPanel.layout().addWidget(
            self.__synoptic, 0, Qt.AlignTop | Qt.AlignHCenter
        )
        self.__topPanel.layout().addWidget(
            self.containerBox, 0, Qt.AlignTop | Qt.AlignHCenter
        )
        ...
        if propertyName == "iconFile":
            ...
            if newValue == "top":
                ...
                self.layout().addWidget(
                    self.__topPanel, 0, Qt.AlignTop | Qt.AlignHCenter
                )
            elif newValue == "bottom":
                self.layout().remove(self.__topPanel)
                self.layout().addWidget(
                    self.__topPanel, 0, Qt.AlignBottom | Qt.AlignHCenter
                )
            else:
                self.layout().remove(self.__topPanel)
