...
class SoleilLoginBrick(BlissWidget):
            ...
            if event.type() == PROPOSAL_GUI_EVENT:
                try:
                    ...
                    logging.getLogger().exception(
                        "SoleilLoginBrick: problem in event! (%s)" % str(diag)
                    )
                    ...
                    if callable(method):
                        try:
                            ...
                            logging.getLogger().exception(
                                "SoleilLoginBrick: uncaught exception! (%s)" % str(diag)
                            )
                        except BaseException:
                            logging.getLogger().exception(
                                "SoleilLoginBrick: uncaught exception!"
                            )
                        else:
                        ...
                        logging.getLogger().warning(
                            "SoleilLoginBrick: uncallable custom event!"
                        )
    ...
    def impersonateProposal(self, proposal_code, proposal_number):
        if BlissWidget.isInstanceUserIdInhouse():
            self._do_login(
                ...
                proposal_number,
                None,
                self.dbConnection.beamline_name,
                impersonate=True,
            )
        else:
            logging.getLogger().debug(
                "SoleilLoginBrick: cannot impersonate unless logged as the inhouse user!"
            )

    # Opens the logout dialog (modal); if the answer is OK then logout the user
    def openLogoutDialog(self):
        logout_dialog = QMessageBox(
            "Confirm logout",
            "Press OK to logout.",
    ...
    def acceptLogin(
        self, proposal_dict, person_dict, lab_dict, session_dict, contact_dict
    ):
        self.setProposal(
            proposal_dict, person_dict, lab_dict, session_dict, contact_dict
        )
        self.setEnabled(True)
    ...
    def askForNewSession(self):
        create_session_dialog = QMessageBox(
            "Create session",
    ...
    def login(self):
        ...
        if self.ldapConnection is None:
            return self.refuseLogin(
                False, "Not connected to LDAP, unable to verify password."
            )
        if self.dbConnection is None:
            return self.refuseLogin(
                False, "Not connected to the ISPyB database, unable to get proposal."
            )
        ...
        logging.getLogger().debug(
            "SoleilLoginBrick login: prop_password %s" % prop_password
        )
        logging.getLogger().debug(
            "SoleilLoginBrick login: self.dbConnection.beamline_name %s"
            % self.dbConnection.beamline_name
        )
    ...
    def _do_login(self, username, proposal_password, beamline_name, impersonate=False):
        ...
        logging.getLogger().debug(
            "SoleilLoginBrick _do_login: proposal_password %s" % proposal_password
        )
        logging.getLogger().debug(
        ...
        if not impersonate:
            ...
            if len(validsess) > 0:
                ...
                projuser = None
            logging.getLogger().debug(
                "SoleilLoginBrick: Number of valid session found: %d" % len(validsess)
            )
            logging.getLogger().debug(
                "SoleilLoginBrick: Project users: %s" % (projuser)
            )
        ...
        logging.getLogger().debug(
            "SoleilLoginBrick: proposal_number: %s" % proposal_number
        )
