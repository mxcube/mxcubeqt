    ...
    def updateBeam(self, force=False):
        if self["displayBeam"]:
            ...
            try:
                ...
                try:
                    # TODO FIXME ERROR: get_beam_info is a function - this cannot be
                    # right
                    # getCommandObject("getBeamInfo")
                    get_beam_info = self.minidiff.getBeamInfo
                    if force or get_beam_info:  # .isSpecReady():
                        self._updateBeam(
                            {"size_x": 0.045, "size_y": 0.025, "shape": "rectangular"}
                        )
                except BaseException:
                    logging.getLogger().exception(
                        "Could not get beam size: cannot display beam"
                    )
                    self.__beam.hide()
            except AttributeError:
                pass
