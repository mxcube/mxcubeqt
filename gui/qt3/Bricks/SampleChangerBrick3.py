...
class SamplesView(QWidget):
    ...
    def setCurrentVial(self, location=None):
        ...
        if (
            self.currentLocation is not None
            and self.currentLocation[0] == self.basketIndex
        ):
            self.vials[self.currentLocation[1] - 1].setPaletteBackgroundColor(
                self.standardColor
            )
            self.numbers[self.currentLocation[1] - 1].setPaletteBackgroundColor(
                self.standardColor
            )
        if location is not None and location[0] == self.basketIndex:
            self.vials[location[1] - 1].setPaletteBackgroundColor(
                SamplesView.CURRENT_VIAL_COLOR
            )
            self.numbers[location[1] - 1].setPaletteBackgroundColor(
                SamplesView.CURRENT_VIAL_COLOR
            )
...
class BasketView(QWidget):
    def __init__(self, parent, basket_index):
        ...
        QObject.connect(
            self.samplesView, PYSIGNAL("loadThisSample"), self.loadThisSample
        )
        QObject.connect(
            self.contentsBox, SIGNAL("toggled(bool)"), self.toggleBasketPresence
        )
...
class CurrentBasketView(CurrentView):
    def __init__(self, parent):
        ...
        QObject.connect(
            self.selected, SIGNAL("valueChanged(int)"), self.selectedChanged
        )
        QObject.connect(
            self.selected.editor(),
            SIGNAL("textChanged(const QString &)"),
            self.selectionTextChanged,
        )
...
class StatusView(QWidget):
    def __init__(self, parent):
        ...
        QObject.connect(
            self.scCanLoad, SIGNAL("clicked()"), self.sampleChangerMoveToLoadingPos
        )
        self.minidiffCanMove = QRadioButton("Minidiff motors can move", self.box2)
        QObject.connect(
            self.minidiffCanMove, SIGNAL("clicked()"), self.minidiffGetControl
        )
...
class ScanBasketsView(QWidget):
    def __init__(self, parent):
        ...
        QObject.connect(
            self.buttonSelect, SIGNAL("clicked()"), self.selectBasketsSamples
        )
...
class SampleChangerBrick3(BlissWidget):
    def __init__(self, *args):
        ...
        QLabel(
            "<b><i>NEW: Mount/unmount samples by right-clicking on the data collection tree on the left</b></i>",
            self.contentsBox,
        )
        self.status = StatusView(self.contentsBox)
        self.cmdSwitchToSampleTransfer = QPushButton(
            "Switch to Sample Transfer mode", self.contentsBox
        )
        ...
        self.cmdResetBasketsSamples = QPushButton(
            "Reset sample changer contents", self.scContents
        )
        QObject.connect(
            self.cmdResetBasketsSamples,
            SIGNAL("clicked()"),
        ...
        self.doubleClickLoads = SCCheckBox(
            "Double-click loads the sample", self.scContents
        )
        ...
        QObject.connect(
            self.status,
            PYSIGNAL("sampleChangerToLoadingPosition"),
            self.sampleChangerToLoadingPosition,
        )
        QObject.connect(
            self.status, PYSIGNAL("minidiffGetControl"), self.minidiffGetControl
        )
        QObject.connect(
            self.status, PYSIGNAL("resetSampleChanger"), self.resetSampleChanger
        )
        QObject.connect(
            self.cmdSwitchToSampleTransfer,
            SIGNAL("clicked()"),
            self.switchToSampleTransferMode,
        )

        QObject.connect(
            self.currentBasket, PYSIGNAL("selectedChanged"), self.changeBasket
        )
        QObject.connect(self.currentBasket, PYSIGNAL("scanBasket"), self.scanBasket)

        QObject.connect(
            self.currentSample, PYSIGNAL("selectedChanged"), self.changeSample
        )
        ...
        QObject.connect(
            self.scanBaskets, PYSIGNAL("scanAllBaskets"), self.scanAllBaskets
        )
        QObject.connect(
            self.scanBaskets,
            PYSIGNAL("selectBasketsSamples"),
            self.selectBasketsSamples,
        )
        QObject.connect(
            self.status, PYSIGNAL("status_msg_changed"), self.status_msg_changed
        )
        ...
        if propertyName == "icons":
            ...
            if self.sampleChanger is not None:
                ...
                self.connect(
                    ...
                    self.sampleChangerStateChanged,
                )
                self.connect(
                    self.sampleChanger,
    ...
    def sampleChangerToLoadingPosition(self):
        if not self.sampleChanger.sampleChangerToLoadingPosition():
            self.status.setSampleChangerLoadStatus(
                self.sampleChanger.sampleChangerCanLoad()
            )
    ...
    def scanBasket(self):
        ...
        self.sampleChanger.scan(
            self.sampleChanger.getSelectedComponent(), recursive=True, wait=False
        )
    ...
    def scanAllBaskets(self):
        ...
        for i, basket_checkbox in enumerate(self.baskets):
            baskets_to_scan.append(
                SC3.Basket.getBasketAddress(i + 1)
    ...
    def infoChanged(self):
        ...
        for basket in baskets:
            presences.append(
                [[VialView.VIAL_UNKNOWN]] * 10
                if basket.isPresent()
                else [[VialView.VIAL_NONE]] * 10
            )
            ...
            if sample.isPresent():
                if matrix:
                    presences[basket_index][vial_index] = (
                        ...
                        else [VialView.VIAL_BARCODE, matrix]
                    )
                else:
                    presences[basket_index][vial_index] = (
                        [VialView.VIAL_NOBARCODE_LOADED, matrix]
                        if sample.hasBeenLoaded()
                        else [VialView.VIAL_NOBARCODE, matrix]
                    )
            else:
...
class BasketsSamplesSelection(QDialog):
    def __init__(self, *args):
        ...
        QObject.connect(
            self.txtPresentBaskets,
            SIGNAL("textChanged(const QString&)"),
            self.presentBasketsChanged,
        )
        ...
        for i in range(5):
            try:
                ...
                if sb.isShown():
                    ...
                    self.result[i + 1] = parse_range(
                        str(self.txtPresentSamples[i].text())
                    )
            except BaseException:
                continue
