...
class DetectorImage(QLabel):
    ...
    def readImage(self):
        if self.waitingImage[3] is not None:
            ...
            full_filename = os.path.join(
                self.waitingImage[0], "%s%s" % (final_filename, template_ext)
            )
    ...
    def readSuccessful(self, filename, data_array, image_headers):
        ...
        qimage, (minVal, maxVal) = pixmaptools.LUT.map_on_min_max_val(
            data_array, palette, pixmaptools.LUT.LOG
        )
...
class DataCollectStatusBrick(BlissWidget):
            ...
            if event.type() == DATACOLLECTSTATUS_GUI_EVENT:
                try:
                    ...
                    logging.getLogger().exception(
                        "DataCollectStatusBrick: problem in event! (%s)" % str(diag)
                    )
                except BaseException:
                    logging.getLogger().exception(
                        "DataCollectStatusBrick: problem in event!"
                    )
                else:
                    ...
                    if callable(method):
                        try:
                            ...
                            logging.getLogger().exception(
                                "DataCollectStatusBrick: uncaught exception! (%s)"
                                % str(diag)
                            )
                        except BaseException:
                            logging.getLogger().exception(
                                "DataCollectStatusBrick: uncaught exception!"
                            )
                        else:
                        ...
                        logging.getLogger().warning(
                            "DataCollectStatusBrick: uncallable custom event!"
                        )
    ...
    def setOscillation(self, osc_id):
        ...
        if self.dbServer is not None and datacollectionid is not None:
            get_collection_thread = GetDataCollectionThread(
                self, datacollectionid, self.dbServer
            )
