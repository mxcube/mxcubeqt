...
#        button_box = qt.QHBoxLayout()
        ...
        for field in fields:
            ...
            # hack until the 'real' xml gets implemented server side and this mess
            # gets rewritten
            if field['type'] == 'message':
                logging.debug('creating widget with options: %s', field)
                w = make_widget(self, field)
                # message will be alone in the layout so that will not fsck up the
                # layout
