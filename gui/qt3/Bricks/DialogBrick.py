...
class DialogBrick(BlissWidget):
    ...
    def propertyChanged(self, propertyName, oldValue, newValue):
        ...
        if self.specversion is not None and self.specWatchVar is not None:
            self.myVar = SpecVariable.SpecVariableA(
                self.specWatchVar,
                self.specversion,
                callbacks={"update": self.spec_update},
            )
    ...
    def spec_update(self, value):
        ...
        if len(messageResult) == 1 or len(messageResult) > 2:
            exec(
                "QMessageBox.%s(self,'Message from Spec (%s)','%s')"
                % (msgType, self.specversion, messageResult)
            )
            self.answer = None
            ...
            exec(
                "self.answer = QMessageBox.%s(self,'Message from Spec (%s)','%s')"
                % (msgType, self.specversion, message)
            )
            ...
            exec(
                "specVar = SpecVariable.SpecVariableA('%s','%s')"
                % (resultVariable, self.specversion)
            )
