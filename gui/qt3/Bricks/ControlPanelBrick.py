...
class ChannelWidgetSettingsDialog(QDialog):
    ...
    def channelConfig(self):
        config = {
            ...
            "label_align": self.optLabelAlignLeft.isChecked()
            and "left"
            or self.optLabelAlignRight.isChecked()
            and "right"
            or "centre",
...
class VerticalGridSpacer(ControlPanelWidget):
    def __init__(self, *args, **kwargs):
        ...
        QHBoxLayout(self)
        self.layoutItem = QSpacerItem(
            10, 10, QSizePolicy.Fixed, QSizePolicy.MinimumExpanding
        )
...
class HorizontalGridSpacer(ControlPanelWidget):
    def __init__(self, *args, **kwargs):
        ...
        QHBoxLayout(self)
        self.layoutItem = QSpacerItem(
            10, 10, QSizePolicy.MinimumExpanding, QSizePolicy.Fixed
        )
        self.layout().addItem(self.layoutItem)
...
class ControlPanelBrick(BlissWidget):
    def __init__(self, *args):
        ...
        self.cmdAddCmd = ToolButton(
            cmdspanel, "Plus2", self.cmdAddCmdClicked, "add command"
        )
        ...
        self.cmdAddCol = ToolButton(
            cmdspanel, "add_col", self.cmdAddColClicked, "add column in grid"
        )
        self.cmdAddRow = ToolButton(
            cmdspanel, "add_row", self.cmdAddRowClicked, "add row in grid"
        )
        HorizontalSpacer(cmdspanel, size=5)
        self.cmdDelCol = ToolButton(
            cmdspanel, "delete_last_col", self.cmdDelColClicked, "delete column"
        )
        self.cmdDelRow = ToolButton(
            cmdspanel, "delete_last_row", self.cmdDelRowClicked, "delete row"
        )
        ...
        self.cmdAddChan = ToolButton(
            chanspanel, "Plus2", self.cmdAddChanClicked, "add channel"
        )
        ...
        HorizontalSpacer(cmdspanel, size=5)
        self.cmdRemove = ToolButton(
            chanspanel, "delete_small", self.cmdRemoveClicked, "remove"
        )
        self.cmdRemove.setEnabled(False)
        HorizontalSpacer(chanspanel)
        self.cmdGridAddVSpacer = ToolButton(
            chanspanel, "vspacer", self.cmdGridAddVSpacerClicked, "add vertical spacer"
        )
        self.cmdGridAddHSpacer = ToolButton(
            chanspanel,
            "hspacer",
    ...
    def readSettings(self):
        ...
        for config in settings:
            ...
            if isinstance(config, types.StringType):
    ...
    def _getSelectedElementIndex(self):
        ...
        if ControlPanelWidget.selected is not None:
            ...
            if selected_element is not None and isinstance(
                selected_element, GridElement
            ):
    ...
    def _addChannel(self, chan_name, i=None, config=None):
        ...
        w = self._addWidget(
            chan_widget_settings_dialog.channelWidget, i=i, running=self.isRunning()
        )
        # print self._widgets_list
    ...
    def _addCommand(self, cmd_name, i=None, config=None):
        ...
        w = self._addWidget(
            cmd_widget_settings_dialog.commandWidget, i=i, running=self.isRunning()
        )
    ...
    def cmdEditClicked(self):
        if self.cmdEdit._type == "channel":
            self._editChannel(
                self.cmdEdit._config["name"],
                self.cmdEdit._i,
                config=self.cmdEdit._config,
            )
        else:
            self._editCommand(
                self.cmdEdit._config["name"],
    ...
    def cmdAddRowClicked(self):
        ...
        while nitems < ngriditems:
            ...
            self._addWidgetToList(
                self._newGridElement(
                    nitems / self["ncols"], (nitems % self["ncols"]) + 1
                ),
                nitems - 1,
            )
