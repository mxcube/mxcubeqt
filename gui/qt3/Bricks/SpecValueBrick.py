...
class SpecValueBrick(BlissWidget):
    ...
    def __init__(self, parent, name):
        ...
        self.nameId = None
        self.addProperty(
            "orientation", "combo", ("horizontal", "vertical"), "horizontal"
        )
    ...
    def buildInterface(self):
        ...
        self.displayWidget = QubValue(
            self, titleType=QubValue.Label, valueType=QubValue.Label, orientation=orient
        )
            ...
            if self.varType is not None:
                if self.varType == "motor":
                    if self.hwId is not None:
                        self.disconnect(
                            self.hwId, qt.PYSIGNAL("positionChanged"), self.valueChanged
                        )
                        self.disconnect(
                            self.hwId, qt.PYSIGNAL("stateChanged"), self.stateChanged
                        )
                else:
                    if self.hwId is not None:
                        self.disconnect(
                            self.hwId, qt.PYSIGNAL("update"), self.valueChanged
                        )
    ...
    def configValue(self):
        ...
        if self.hwId is not None and self.varType is not None:
            if self.varType == "motor":
                ...
                self.connect(self.hwId, qt.PYSIGNAL("stateChanged"), self.stateChanged)
            else:
                self.connect(self.hwId, qt.PYSIGNAL("update"), self.valueChanged)
