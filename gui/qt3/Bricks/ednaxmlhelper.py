...
def get_field_containers(xml_root):
    if isinstance(xml_root, types.StringType):
...
def get_fields(field_container):
    all_fields = list()
    #this is ridiculous
    fields = field_container.xpath(
        './void[@property="fields"]/void[@method="add"]/object[@class="org.dawb.passerelle.actors.ui.config.FieldBean"]')
    ...
    for field in fields:
        ...
        if field_dict['type'] == 'combo':
            ...
            # please kill me
            vals = field.xpath(
                './void[@property="textChoices"]/void[@method="add"]/object[@class="org.dawb.passerelle.actors.ui.config.StringValueBean"]/void[@property="textValue"]/string')
            for val in vals:
