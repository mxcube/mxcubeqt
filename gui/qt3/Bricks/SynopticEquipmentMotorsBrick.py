        ...
        if self.hardwareObject is None:
            ...
            if self.hardwareObject.hasObject("motors"):
                ...
                logging.getLogger().warning(
                    "Equipment %s does not have motors", self.hardwareObject.userName()
                )
        ...
        if displayMode == "tabs":
            ...
            for motorCategory in motors.objectsNames():
                ...
                for motor in motors[motorCategory].getDevices():
                    ...
                    newMotorWidget.getProperty("mnemonic").setValue(
                        str(motor.name())
                    )  # ['mnemonic'] = str(motor.name())
                    ...
                    newMotorWidget.getProperty("allowDoubleClick").setValue(
                        self["allowControl"]
                    )
                    newMotorWidget.getProperty("dialogCaption").setValue(
                        "%s [%s] - %s"
                        % (self["title"], motorCategory, motor.userName())
                    )
