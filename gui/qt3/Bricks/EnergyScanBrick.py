...
class EnergyScanBrick(BlissWidget):
    STATES = {"error": QWidget.red, "ok": QWidget.green, "progress": QWidget.yellow}
    ...
    def __init__(self, *args):
        ...
        QObject.connect(
            self.prefixInput, SIGNAL("textChanged(const QString &)"), self.prefixChanged
        )
        ...
        self.connect(
            self.startScanButton, PYSIGNAL("executeCommand"), self.startEnergyScan
        )
        self.connect(
            self.startScanButton, PYSIGNAL("cancelCommand"), self.cancelEnergyScan
        )
    ...
    def browseButtonClicked(self):
        ...
        d = get_dir.getExistingDirectory(
            self.directoryInput.text(), self, "", "Select a directory", True, False
        )
    ...
    def propertyChanged(self, propertyName, oldValue, newValue):
        if propertyName == "mnemonic":
            if self.energyScan is not None:
                ...
                self.disconnect(
                    self.energyScan, "energyScanFinished", self.scanFinished
                )
                self.disconnect(self.energyScan, "energyScanFailed", self.scanFailed)
                self.disconnect(
                    self.energyScan, "scanStatusChanged", self.scanStatusChanged
                )
    ...
    def startEnergyScan(self):
        ...
        scan_result = self.energyScan.startEnergyScan(
            self.element[0],
            self.element[1],
