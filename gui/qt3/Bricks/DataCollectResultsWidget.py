...
class DetectorImage(VideoDisplay.VideoDisplayWidget):
    ...
    def changeContrast(self, i):
        ...
        if self.currentImage[0] is not None:
            self.setWantedImage(
                self.currentImage[0], self.currentImage[1], self.currentImage[2], False
            )
    ...
    def wantedImageUpdated(self, delay=True):
        ...
        full_filename = os.path.join(
            self.wantedImage[0], "%s%s" % (final_filename, template_ext)
        )
        self.waitingFor = [
            full_filename,
            None,
    ...
    def setImageServer(self, image_server):
        ...
        if self.imageServer is not None:
            ...
            self.imageServer.disconnect(
                "imageTransferCompleted", self.imageTransferCompleteqqqqd
            )
    ...
    def imageReceived(self, jpeg_data):
        ...
        if display:
            ...
            self.emit(
                PYSIGNAL("imageUpdated"), (self.waitingFor[0], self.waitingFor[4], True)
            )
            self.waitingFor = [None, None, None, None, None]
            self.wantedImageUpdated()
