...
class CommandMenuBrick(BlissWidget):
        ...
        if property == "mnemonic":
            ...
            if self.safetyShutter is None:
                logging.getLogger().warning(
                    "CommandMenuBrick: safety shutter not configured"
                )
        elif property == "icons":
    ...
    def isSearchDone(self):
        if self.safetyShutter is not None:
            if self.safetyShutter.getShutterState() in (
                "disabled",
                "fault",
                "error",
                "moving",
    ...
    def executeCommand(self):
        ...
        if not len(self.arguments):
            if self.confirmationMsg is not None:
                ...
                conf_msg = (
                    "The hutch hasn't been searched! Press OK to continue anyway."
                )
                self.confirmDialog = QMessageBox(
                    self.cmd.userName(),
                    conf_msg,
            ...
            if not searched:
                conf_msg = (
                    "<b>%s</b><br>Warning: the hutch hasn't been searched!"
                    % self.confirmationMsg
                )
            else:
        ...
        if go:
            ...
            if len(self.arguments) > 0:
                if self.argumentsDialog is None:
                    self.argumentsDialog = ArgumentsDialog(
                        self, self.cmd.userName(), self.arguments, conf_msg
                    )
                else:
            ...
            if res == QMessageBox.Ok:
                ...
                for arg in self.arguments:
                    ...
                    try:
                        ...
                        problems.append(
                            "The value <%s> isn't a valid %s for %s."
