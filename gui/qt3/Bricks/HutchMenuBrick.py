...
from Qub.Objects.QubDrawingManager import (
    QubPointDrawingMgr,
    Qub2PointSurfaceDrawingMgr,
    QubAddDrawing,
)
...
class HutchMenuBrick(BlissWidget):
    ...
    def __init__(self, *args):
        ...
        self.connect(
            self.buttonCentre, PYSIGNAL("executeCommand"), self.centreSampleClicked
        )
        self.connect(
            self.buttonCentre, PYSIGNAL("cancelCommand"), self.cancelCentringClicked
        )
        ...
        QObject.connect(
            self.buttonToogleMDPhase, SIGNAL("clicked()"), self.toggleMDPhase
        )
    ...
    def propertyChanged(self, propertyName, oldValue, newValue):
        if propertyName == "beamInfo":
            if self.beamInfo is not None:
                self.disconnect(
                    self.beamInfo, PYSIGNAL("beamInfoChanged"), self.beamInfoChanged
                )
                self.disconnect(
                    self.beamInfo, PYSIGNAL("beamPosChanged"), self.beamPosChanged
                )
            ...
            if self.minidiff is not None:
                self.disconnect(
                    self.minidiff, PYSIGNAL("minidiffReady"), self.miniDiffReady
                )
                self.disconnect(
                    self.minidiff, PYSIGNAL("minidiffNotReady"), self.miniDiffNotReady
                )
                self.disconnect(
                    self.minidiff,
                ...
                self.disconnect(
                    self.minidiff, PYSIGNAL("centringStarted"), self.centringStarted
                )
                self.disconnect(
                    self.minidiff,
                ...
                self.disconnect(
                    self.minidiff, PYSIGNAL("centringAccepted"), self.centringAccepted
                )
                self.disconnect(
                    self.minidiff, PYSIGNAL("centringFailed"), self.centringFailed
                )
                self.disconnect(
                    self.minidiff, PYSIGNAL("centringMoving"), self.centringMoving
                )
                self.disconnect(
                    self.minidiff, PYSIGNAL("centringInvalid"), self.centringInvalid
                )
                self.disconnect(
                    self.minidiff, PYSIGNAL("centringSnapshots"), self.centringSnapshots
                )
                self.disconnect(
                    self.minidiff, PYSIGNAL("progressMessage"), self.miniDiffMessage
                )
                self.disconnect(
                    self.minidiff,
    ...
    def saveSnapshot(self):
        ...
        if len(filename):
            ...
            try:
                ...
                QubImageSave.save(
                    filename, img, self.__drawing.canvas(), zoom, image_type
                )
            except BaseException:
    ...
    def imageClicked(self, x, y, xi, yi):
        if (
            self.currentCentring is not None
            ...
            if self.minidiff and self.minidiff.isReady():
                try:
                    ...
                    self.emit(
                        ...
                        (1e3 / self.pixels_per_mm[0], 1e3 / self.pixels_per_mm[1]),
                    )
                except BaseException:
                    pass
    ...
    def run(self):
        ...
        try:
            ...
            try:
                ...
                self.emit(
                    PYSIGNAL("calibrationChanged"), (self.__scaleX, self.__scaleY)
                )
    ...
    def zoomPositionChanged(self, position, offset):
        ...
        if offset is None:
            ...
            if self.minidiff is not None:
                self.pixels_per_mm = self.minidiff.get_pixels_per_mm()
                if (
                    self.pixels_per_mm[0] is not None
