...
class XfeSpectrumBrick(BlissWidget):
    ...
    def __init__(self, *args):
        ...
        self.connect(
            self.startSpectrumButton,
            PYSIGNAL('executeCommand'),
            self.startXfeSpectrum)
        self.connect(
            self.startSpectrumButton,
            PYSIGNAL('cancelCommand'),
            self.cancelXfeSpectrum)
        ...
        self.spectrumStatus.setSizePolicy(
    ...
    def startXfeSpectrum(self):
        ...
        if self.sessionId is None or self.sessionId == "":
            res = QMessageBox.question(
                self,
                'XRF Spectrum',
                "You are not properly logged, therefore your spectrum won't be stored in ISPyB.\nContinue?",
                "Yes",
                "No")
            if res != 0:
                go_on = False
    ...
    def spectrumFinished(self, mca_data, calib, config):
        ...
        filename_pattern = os.path.join(str(self.directoryInput.text()), "%s_%s_%%02d" % (
