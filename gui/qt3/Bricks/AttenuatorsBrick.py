...
class AttenuatorsBrick(BlissWidget):
    ...
    def __init__(self, *args):
        ...
        self.addProperty(
            "filtersMode", "combo", ("Expert", "Enabled", "Disabled"), "Expert"
        )
    ...
    def inputFieldChanged(self, text):
        text = str(text)
        if text == "":
            self.newTransmission.setPaletteBackgroundColor(
                AttenuatorsBrick.CONNECTED_COLOR
            )
        else:
            try:
                val = float(text)
    ...
    def filtersChanged(self, value):
        try:
            ...
            logging.getLogger().warning(
                "AttenuatorsBrick: error reading filter status (%d)" % value
            )
