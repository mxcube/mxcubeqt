    ...
    def submitMessage(self):
        ...
        try:
            ...
            logging.getLogger().debug(
                "Sending feedback from %s to %s" % (fromaddr, toaddrs)
            )
...
class LogViewBrick(BaseComponents.BlissWidget):
        ...
        for l in LogViewBrick.LOGS:
            if l == "Feedback":
                self.FeedbackLog = SubmitFeedback(
                    self, LogViewBrick.TITLES["Feedback"], self["emailAddresses"]
                )
            else:
    ...
    def run(self):
        ...
        # GUILogHandler.GUILogHandler().register(self)
        self.connect(
            self.tabs, SIGNAL("currentChanged(QWidget *)"), self.resetUnreadMessages
        )
                ...
                if newValue:
                    if self.tabs.indexOf(self.DebugLog) == -1:
                        ...
                        self.tabs.setTabToolTip(
                            self.DebugLog, LogViewBrick.TOOLTIPS["Debug"]
                        )
                else:
                    if self.tabs.indexOf(self.DebugLog) != -1:
            ...
            if self["appearance"] == "tabs":
                if newValue:
                    if self.tabs.indexOf(self.FeedbackLog) == -1:
                        self.tabs.insertTab(
                            self.FeedbackLog, self.FeedbackLog.tabLabel, 2
                        )
                        self.tabs.setTabToolTip(
                            self.FeedbackLog, LogViewBrick.TOOLTIPS["Feedback"]
                        )
                else:
                    if self.tabs.indexOf(self.FeedbackLog) != -1:
            ...
            if newValue == "list":
                ...
                for l in LogViewBrick.LOGS:
                    if l == "Debug" and not self["showDebug"]:
                        ...
                        exec(
                            "self.tabs.addTab(self.%sLog,'%s')"
                            % (l, LogViewBrick.TITLES[l])
                        )
                        exec(
                            "self.tabs.setTabToolTip(self.%sLog,'%s')"
                            % (l, LogViewBrick.TOOLTIPS[l])
                        )
