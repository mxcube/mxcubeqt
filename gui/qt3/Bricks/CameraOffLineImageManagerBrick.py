...
class CameraOffLineImageManagerBrick(BlissWidget):
    ...
    def ChangePixelCalibration(self, sizeX, sizeY):
        if sizeX is not None and sizeY is not None:
            ...
            if motorXUnit is None:
                motorXUnit = 1e-3

            motorYUnit = self.__vMotor.getProperty('unit')
            if motorYUnit is None:
                motorYUnit = 1e-3
        ...
        if self.__master_motor is not None:
            ...
            try:
                ...
                master_item = _MasterCheckItem(
                    self.__masterControler,
                    'p_%d (%s)' %
                    (len(
                        self.__masterPosition2Item),
                        positionString))
    ...
    def run(self):
        ...
        try:
            ...
            self.__saveMosaicAction = _openDialog(
                parent=self,
                label='Save image',
                name="save",
                iconName='save',
                group="admin")
            saveMosaicDialogue = qt.QFileDialog(
                '.', 'Mosaic Images (*.svg)', self, 'Save mosaic Images', True)
            ...
            if self.__vMotor is not None:
                self.__imageMosaicPosition.setLabel(
                    QubRulerAction.VERTICAL, 0, self.__vMotor.getMotorMnemonic())
                self.__imageMosaicPosition.setCursorPosition(QubRulerAction.VERTICAL, 0,
            ...
            if self.__hMotor is not None:
                self.__imageMosaicPosition.setLabel(
                    QubRulerAction.HORIZONTAL, 0, self.__hMotor.getMotorMnemonic())
...
class _SavePickleItem:
    def __init__(self, checkitem, image, layer, state,
                 cnt, masterPosition, focusPosition):
        self.image = image
        self.textcolumn = []
    ...
    def __init__(self, parent, name, cnt, image=None,
        ...
        if isinstance(name, _SavePickleItem):
            if lastItem:
                ...
                qt.QCheckListItem.__init__(
                    self, parent, name.textcolumn[0], qt.QCheckListItem.CheckBox)
            saveItem = name
            ...
            self.setState(
                saveItem.masterPosition and qt.QCheckListItem.On or qt.QCheckListItem.Off)
            self.setRenameEnabled(0, True)
        else:
            self.__image = QubMosaicImage(
                image, hMotor.getPosition(), vMotor.getPosition(), 0)
            if lastItem:
                qt.QCheckListItem.__init__(
                    self, parent, lastItem, name, qt.QCheckListItem.CheckBox)
            else:
                qt.QCheckListItem.__init__(
                    self, parent, name, qt.QCheckListItem.CheckBox)
