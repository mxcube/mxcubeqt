...
class ChatBrick(BaseComponents.BlissWidget):
    ...
    def __init__(self, *args):
        ...
        QObject.connect(
            self.message, SIGNAL("textChanged(const QString &)"), self.messageChanged
        )
    ...
    def loadChatHistory(self):
        if self.instanceServer is not None:
            chat_history_filename = "/tmp/mxCuBE_chat_%s.%s" % (
                self.session_id,
                self.instanceServer.isClient() and "client" or "server",
            )
        else:
            return
        try:
    ...
    def setRole(self, role):
        self.role = role
        if (
            role != BaseComponents.BlissWidget.INSTANCE_ROLE_UNKNOWN
    ...
    def newClient(self, client_id):
        msg = "%s has joined the conversation." % self.instanceServer.idPrettyPrint(
            client_id
        )
    ...
    def clientClosed(self, client_id):
        msg = "%s has left the conversation..." % self.instanceServer.idPrettyPrint(
            client_id
        )
        self.messageArrived(InstanceServer.ChatInstanceMessage.PRIORITY_LOW, None, msg)
    ...
    def propertyChanged(self, propertyName, oldValue, newValue):
        if propertyName == "mnemonic":
            if self.instanceServer is not None:
                ...
                self.disconnect(
                    self.instanceServer, PYSIGNAL("serverClosed"), self.clientClosed
                )
                self.disconnect(
