...
class InstanceListBrick(BlissWidget):
    ...
    def __init__(self, *args):
        ...
        QObject.connect(
            self.listBox, SIGNAL("pressed(QListBoxItem *)"), self.giveControlTo
        )
        self.connections = {}
        ...
        QObject.connect(
            self.takeControlButton, SIGNAL("clicked()"), self.takeControlClicked
        )
        ...
        QObject.connect(
            self.askForControlButton, SIGNAL("clicked()"), self.askForControlClicked
        )
    ...
    def propertyChanged(self, propertyName, oldValue, newValue):
        if propertyName == "mnemonic":
            if self.instanceServer is not None:
                self.disconnect(
                    self.instanceServer,
                    PYSIGNAL("instanceInitializing"),
                    self.instanceInitializing,
                )
                self.disconnect(
                    self.instanceServer,
                    PYSIGNAL("serverInitialized"),
                    self.serverInitialized,
                )
                self.disconnect(
                    self.instanceServer,
                    PYSIGNAL("clientInitialized"),
                    self.clientInitialized,
                )
                self.disconnect(
                    self.instanceServer, PYSIGNAL("serverClosed"), self.serverClosed
                )
                self.disconnect(
                    self.instanceServer, PYSIGNAL("newClient"), self.newClient
                )
                self.disconnect(
                    self.instanceServer, PYSIGNAL("haveControl"), self.haveControl
                )
                self.disconnect(
                    self.instanceServer, PYSIGNAL("passControl"), self.passControl
                )
                self.disconnect(
                    self.instanceServer, PYSIGNAL("wantsControl"), self.wantsControl
                )
                self.disconnect(
                    self.instanceServer, PYSIGNAL("widgetUpdate"), self.widgetUpdate
                )
                self.disconnect(
                    self.instanceServer, PYSIGNAL("clientChanged"), self.clientChanged
                )
                self.disconnect(
                    self.instanceServer, PYSIGNAL("clientClosed"), self.clientClosed
                )
                self.disconnect(
                    self.instanceServer, PYSIGNAL("widgetCall"), self.widgetCall
                )
    ...
    def hutchTriggerChanged(self, hutch_opened):
        if hutch_opened:
            if not BlissWidget.isInstanceRoleServer():
                ...
                logging.getLogger().info(
                    "%s: HUTCH IS OPENED, TAKING CONTROL OVER REMOTE USERS", self.name()
                )
    ...
    def setSession(
        ...
        if (
            ...
            if is_inhouse:
                ...
                self.takeControlButton.setEnabled(BlissWidget.isInstanceRoleServer())
                if (
                    self.hutchtrigger is not None
                    ...
                    logging.getLogger().info(
                        ...
                        self.name(),
                        hutch_opened and "opened" or "close",
    ...
    def instanceInitializing(self):
        ...
        self.connect(
            qApp.mainWidget(),
            PYSIGNAL("applicationTabChanged"),
            self.applicationTabChanged,
        )

    def clientInitialized(
    ...
    def serverInitialized(self, started, server_id=None):
        if started:
            ...
            msg_dialog = QMessageBox(
                ...
                QMessageBox.NoButton,
                self,
            )  # Application name (mxCuBE) is hardwired!!!
    ...
    def widgetCall(self, timestamp, method, method_args):
        try:
            ...
            logging.getLogger().debug(
                "InstanceListBrick: problem executing an external call"
            )
    ...
    def applicationBrickChanged(
        self, brick_name, widget_name, method_name, method_args, masterSync
    ):
    ...
    def takeControlClicked(self):
        ...
        take_control_dialog = QMessageBox(
            "mxCuBE",
            "You're about to take control of the application, taking over %s!"
            % control_user_print,
    ...
    def instanceUserIdChanged(self, userid):
        logging.getLogger().info(
            ...
            % InstanceListBrick.IDS[userid].replace("_", " ").lower()
        )
        self.updateMirroring()
    ...
    def clientClosed(self, client_id):
        ...
        try:
            ...
            logging.getLogger().warning(
                "InstanceListBrick: unknown client has closed (%s)" % str(client_id)
            )
        else:
        ...
        if BlissWidget.isInstanceModeSlave():
            if BlissWidget.isInstanceUserIdUnknown():
                if (
                    ...
                    and self.inControl is not None
                    and self.inControl[1] is None
                ):
    ...
    def timeoutApproaching(self):
        if self.giveControlDialog is not None:
            if self.timeoutLeft in (30, 20, 10):
                self.instanceServer.sendChatMessage(
                    InstanceServer.ChatInstanceMessage.PRIORITY_LOW,
                    "%s will have control in %d seconds..."
                    % (self.giveControlDialog.nickname, self.timeoutLeft),
                )
            self.timeoutLeft -= 1
            if self.timeoutLeft == 0:
                self.giveControlDialog.done(QMessageBox.Yes)
            else:
                self.giveControlDialog.setButtonText(
                    QMessageBox.Yes, "Allow (%d secs)" % self.timeoutLeft
                )
        ...
        if self.isRunning():
            if event.type() == WANTS_CONTROL_EVENT:
                try:
                    ...
                    logging.getLogger().exception(
                        "InstanceListBrick: problem in event!"
                    )
                else:
                    client_print = self.instanceServer.idPrettyPrint(client_id)
                ...
                self.instanceServer.sendBrickUpdateMessage(
                    event.brick_name,
                    event.widget_name,
                    event.method_name,
                    event.method_args,
                ...
                self.instanceServer.sendTabUpdateMessage(
    ...
    def wantsControl(self, client_id):
        ...
        if (
            BlissWidget.isInstanceModeMaster()
            and self.giveControlDialog is None
            and self.allowTimeoutControl.isChecked()
        ):
            self.timeoutLeft = self["giveControlTimeout"]
            client_print = self.instanceServer.idPrettyPrint(client_id)
            self.giveControlDialog = QMessageBox(
                "Pass control",
...
class ExternalUserInfoDialog(QDialog):
    def __init__(self):
        ...
        self.message2 = QLabel(
            "<nobr>Please enter the following information, <u>required</u> for the experimental hall operator:",
            self.messageBox,
        )
        ...
        self.buttonsBox = DialogButtonsBar(
            self,
            "Continue",
            None,
            None,
        ...
        QVBoxLayout(
            self, DialogButtonsBar.DEFAULT_MARGIN, DialogButtonsBar.DEFAULT_SPACING
        )
    ...
    def validateParameters(self):
        if (
            len(str(self.nameInput.text()))
            and len(str(self.instituteInput.text()))
            and len(str(self.phoneInput.text()))
            and len(str(self.emailInput.text()))
        ):
