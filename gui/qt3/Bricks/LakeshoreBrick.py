...
class LakeshoreBrick(BlissWidget):
    def __init__(self, *args):
        ...
        self.lblUpdateFrequency = QLabel(
            "<nobr><b>current = ?</b></nobr>", updateFreqBox
        )
        ...
        QObject.connect(
            self.cmdUpdateFrequency, SIGNAL("clicked()"), self.lsUpdateFrequency
        )
        ...
        if property == "mnemonic":
            ...
            if self.lakeshore is not None:
                ...
                for i in range(self.lakeshore.getChannelsNumber()):
                    ...
                    QObject.connect(
                        newCheckbox, SIGNAL("stateChanged(int)"), checkbox_cb
                    )
    ...
    def lsChannelsUpdated(self, values):
        ...
        if self["baseTime"] == "0":
            ...
            try:
                t0 = min(
                    filter(
                        None, [d["t0"] for d in filter(None, self.data.itervalues())]
                    )
                )
            except ValueError:
                t0 = t
