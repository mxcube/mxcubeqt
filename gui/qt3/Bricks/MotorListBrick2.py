...
class MotorListBrick2(BaseComponents.BlissWidget):
    def __init__(self, *args):
        ...
        QToolTip.add(
            self.openButton, "Opens a dialog box to control the selected motor"
        )
    ...
    def openCurrentMotor(self):
        ...
        try:
            ...
            if motor_username not in self.dialogs:
                m = SpecMotor.SpecVersionMotor(
                    self.listObj.getSpecVersion(), motor_specname, motor_username
                )
    ...
    def setExpertMode(self, state):
        ...
        if self["disableInUser"]:
            if state:
                ...
                QToolTip.add(
                    self.containerBox, "Select the motor you desire to control"
                )
            else:
                ...
                QToolTip.add(
                    self.containerBox,
                    "Only in expert mode you're allowed to change this motor",
                )
