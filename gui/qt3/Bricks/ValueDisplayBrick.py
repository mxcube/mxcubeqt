...
class ValueDisplayBrick(SynopticBrick.SynopticBrick):
    def __init__(self, *args):
        ...
        self.addProperty(
            "displayType", "combo", ("string", "LCD (number)"), "LCD (number)"
        )
        ...
        if value is None:
            ...
            if isinstance(value, types.IntType) or isinstance(value, types.FloatType):
                ...
                logging.getLogger().error(
                    "%s: cannot display value, unknown type %s",
                    str(self.name()),
                    type(value),
                )
    ...
    def propertyChanged(self, propertyName, oldValue, newValue):
        if propertyName == "mnemonic":
            if self.tacoHardwareObject is not None:
                self.disconnect(
                    self.tacoHardwareObject, PYSIGNAL("valueChanged"), self.setValue
                )
                self.disconnect(
                    self.tacoHardwareObject, PYSIGNAL("timeout"), self.setValue
                )
