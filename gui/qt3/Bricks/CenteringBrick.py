...
from Qub.Objects.QubDrawingCanvasTools import (
    QubCanvasVLine,
    QubCanvasHLine,
    QubCanvasTarget,
)
...
class CenteringBrick(BlissWidget):
        ...
        if property == "mnemonic":
            ...
            if equipment is not None:
                ...
                if xoryMotor is not None:
                    ...
                    if xoryMotor is None:
                        logging.getLogger().error(
                            "%s: could not find motors horizontal nor vertical motor in Hardware Objects %s",
                            str(self.name()),
                            equipment.name(),
                        )
                        return
                ...
                if zMotor is None or rMotor is None:
                    logging.getLogger().error(
                        "%s: could not find motors inBeam or rotation motor in Hardware Objects %s",
                        str(self.name()),
                        equipment.name(),
                    )
                    return
                ...
                self.__centeringPlug = _centeringPlug(
                    self,
                    self.__widgetTree,
                    xoryMotor,
                    zMotor,
                    ...
                    table_y,
                    table_z,
                    self.getProperty("table_y_inverted"),
                    self.getProperty("table_z_inverted"),
                )
    ...
    def multiPointCentre(self, z, phis):
        ...
        p1, sucess = optimize.leastsq(errfunc, [1.0, 0.0, 0.0], args=(phis, z))
        return p1
...
class _secondStepAlignementProcess(_alignementProcess):
    ...
    def __init(self):
        self._startDraw()
        self._lastDrawing.setActionInfo(
            "In alignement procedure, select %d point" % (self.__nbPoint + 1)
        )
        self._phiStep()
