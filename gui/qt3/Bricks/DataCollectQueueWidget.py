...
class editItem(qttable.QTableItem):
    ...
    def createEditor(self):
        ...
        try:
            QObject.connect(
                self.myLineEdit, PYSIGNAL("inputValid"), self.processValidFun
            )
        except AttributeError:
            pass
...
class DataCollectQueueWidget(QWidget):
    ...
    PARAMETERS = {
        ...
        "osc_start": (
            ...
            "Osc.start",
            None,
            [editItem, qttable.QTableItem.Always],
            (QDoubleValidator,),
        ),
        "osc_range": (
            2,
        ...
        "inverse_beam": (
            ...
            "Inverse beam",
            None,
            [boolEditItem, qttable.QTableItem.Always],
            (optionalIntValidator, 1),
        ),
        "run_number": (
            11,
            "Run number",
        ...
        "directory": (
            ...
            "Directory",
            None,
            [directoryItem, qttable.QTableItem.Always],
            [directoryValidator, None, None],
        ),
    ...
    def setDetectorType(self, detector_type):
        if detector_type == "adsc":
            ...
            self.table.setColumnReadOnly(
                DataCollectQueueWidget.PARAMETERS["detector_mode"][0], True
            )
    ...
    def addParamDict(self, sample_info, collect_params, extra_parameters):
        ...
        try:
            ...
            try:
                collect_params2["transmission"] = extra_parameters[
                ...
                if param_type is not None:
                    ...
                    if param_validator is not None:
                        ...
                        try:
                            ...
                            if param_class_validator2 is not None:
                                obj_validator.setSpecificValidator(
                                    param_class_validator2
                                )
                    ...
                    if self.table.isColumnReadOnly(param_index):
                        ...
                        if param_name == "transmission":
                            param_obj.setQueueItemValue(
                                extra_parameters["fixed_transmission"]
                            )
                        elif param_name == "energy":
                            param_obj.setQueueItemValue(
                                extra_parameters["fixed_energy"]
                            )
    ...
    def addParamList(self, selected_samples, collect_params, extra_parameters):
        ...
        try:
            ...
            self.table.setColumnReadOnly(
                DataCollectQueueWidget.PARAMETERS["energy"][0], True
            )
        else:
            self.table.setColumnReadOnly(
                DataCollectQueueWidget.PARAMETERS["energy"][0], False
            )
        try:
            current_transmission = extra_parameters["current_transmission"]
        except KeyError:
            self.table.setColumnReadOnly(
                DataCollectQueueWidget.PARAMETERS["transmission"][0], True
            )
            extra_parameters["fixed_transmission"] = 100
        else:
            self.table.setColumnReadOnly(
                DataCollectQueueWidget.PARAMETERS["transmission"][0], False
            )
        try:
            current_resolution = extra_parameters["current_resolution"]
        except KeyError:
            self.table.setColumnReadOnly(
                DataCollectQueueWidget.PARAMETERS["resolution"][0], True
            )
        else:
            self.table.setColumnReadOnly(
                DataCollectQueueWidget.PARAMETERS["resolution"][0], False
            )
                ...
                for collect_param in collect_params:
                    if is_multiple:
                        ...
                        if blsampleid is not None and blsampleid != "":
                            collect_param["prefix"] = "%s-%s" % (
                                sel_sample[1]["protein_acronym"],
                                sel_sample[1]["name"],
                            )
                            collect_param["directory"] = os.path.join(
                                collect_param["directory"],
                                sel_sample[1]["protein_acronym"],
                                sel_sample[1]["name"],
                            )
                        else:
                            ...
                            collect_param["directory"] = os.path.join(
                                collect_param["directory"], sel_sample[1]["code"]
                            )
    ...
    def updateLength(self):
        count = len(self.queueList)
        self.collectButton.setEnabled(
            self.lastSpecReady and count > 0 and self.lastValid
        )
