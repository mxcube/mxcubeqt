...
class MicrodiffBrick(BlissWidget):
    def __init__(self, *args):
        ...
        QObject.connect(
            self.chkBackground, SIGNAL("clicked()"), self.setAutocheckBackground
        )
    ...
    def propertyChanged(self, property, oldValue, newValue):
        if property == "mnemonic":
            if self.microdiff is not None:
                self.disconnect(
                    self.microdiff, PYSIGNAL("stateChanged"), self.microdiffStateChanged
                )
                self.disconnect(
                    self.microdiff,
                    PYSIGNAL("statusChanged"),
                    self.microdiffStatusChanged,
                )
                self.disconnect(
                    self.microdiff,
                    PYSIGNAL("centeringSuccessful"),
                    self.microdiffCenteringSuccessful,
                )
                self.disconnect(
                    self.microdiff,
                    PYSIGNAL("autocheckBackground"),
                    self.autocheckBackgroundChanged,
                )
    ...
    def microdiffStatusChanged(self, status):
        ...
        if status.startswith("centring") and status.endswith("successfull"):
            ...
            self.emit(
                PYSIGNAL("centringSuccessful"), (1, self.microdiff.getCentringStatus())
            )
        # else:
