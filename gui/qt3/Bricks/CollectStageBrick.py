...
STAGE_COLORS = {"UNKNOWN": None, "WORKING": "yellow", "DONE": "green", "ERROR": "red"}

STATE_DICT = {None: "WORKING", False: "ERROR", True: "DONE"}
...
class CollectStageBrick(BlissWidget):
    ...
    def propertyChanged(self, property, oldValue, newValue):
        if property == "mnemonic":
            if self.dataCollect is not None:
                ...
                self.disconnect(
                    self.dataCollect, "collectNumberOfFrames", self.collectSpecStarted
                )
    ...
    def collectOscillationStarted(
        self, owner, blsampleid, barcode, location, collect_dict, oscillation_id, *args
    ):
        self.clearStages()
        self.setStage(1, None)
    ...
    def setStage(self, stage_index, state):
        ...
        exec(
            "self.pic%d.setPaletteBackgroundColor(QColor('%s'))"
            % (stage_index, STAGE_COLORS[STATE_DICT[state]])
        )
        exec(
            "self.label%d.setPaletteBackgroundColor(QColor('%s'))"
            % (stage_index, STAGE_COLORS[STATE_DICT[state]])
        )
