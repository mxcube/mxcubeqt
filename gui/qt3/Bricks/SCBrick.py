...
class SCBrick(BaseComponents.BlissWidget):
    def __init__(self, *args):
        ...
        qt.QObject.connect(
            self.widget.btOperMode, qt.SIGNAL("clicked()"), self._setOperMode
        )
        qt.QObject.connect(
            self.widget.btChargeMode, qt.SIGNAL("clicked()"), self._setChargeMode
        )
                ...
                if element.isLeaf():
                    ...
                    if img_str is not None:
                        ...
                        if (x is not None) and (y is not None):
                            ...
                            p.drawLine(
                                qt.QPoint(center.x(), center.y() - size),
                                qt.QPoint(center.x(), center.y() + size),
                            )
                            p.drawLine(
    ...
    def onListPopupMenu(self, item, point, col):
        ...
        if col == 0 and (component is not None):
            ...
            if component.isLeaf():
                menu.insertItem(
                    "Load from", self.onMenuLoad
                ), ready and component.isLeaf()
                menu.insertItem(
                    "Unload to", self.onMenuUnload
                ), ready and component.isLeaf()
            menu.popup(point)
    ...
    def _updateButtons(self):
        ...
        if self.device is None or not self.device.isReady():
            ...
            self.widget.btLoadSample.setEnabled(
                ready and not self.device.hasLoadedSample() and not charging
            )
            self.widget.btUnloadSample.setEnabled(
                ready and self.device.hasLoadedSample() and not charging
            )
