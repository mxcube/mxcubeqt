...
class MultiplePositionsBrick(BlissWidget):
    ...
    def buildInterface(self):
        ...
        self.connect(
            self.radioGroup, qt.PYSIGNAL("PositionClicked"), self.positionClicked
        )
        ...
        self.connect(self.gotoButton, qt.SIGNAL("clicked()"), self.gotoPosition)
    ...
    def appearanceChange(self, appearance):
        try:
            ...
            if (
                self.appearance == "Move"
                or self.appearance == "Configure"
                or self.appearance == "Incremental"
            ):
    ...
    def connectHardwareObject(self):
        if self.hwro is not None:
            self.connect(
                self.hwro, qt.PYSIGNAL("positionReached"), self.positionChanged
            )
            self.connect(self.hwro, qt.PYSIGNAL("noPosition"), self.noPosition)
            self.connect(self.hwro, qt.PYSIGNAL("stateChanged"), self.checkState)
            self.connect(self.hwro, qt.PYSIGNAL("equipmentReady"), self.equipmentReady)
            self.connect(
                self.hwro, qt.PYSIGNAL("equipmentNotReady"), self.equipmentNotReady
            )
    ...
    def disconnectHardwareObject(self):
        ...
        if self.hwro is not None:
            ...
            self.disconnect(self.hwro, qt.PYSIGNAL("noPosition"), self.noPosition)
            self.disconnect(self.hwro, qt.PYSIGNAL("stateChanged"), self.checkState)
            self.disconnect(
                self.hwro, qt.PYSIGNAL("equipmentReady"), self.equipmentReady
            )
            self.disconnect(
                self.hwro, qt.PYSIGNAL("equipmentNotReady"), self.equipmentNotReady
            )
    ...
    def positionClicked(self, name):
        if self.hwro is not None:
            if self.checkMove:
                msgstr = 'You will move "%s" to position "%s"' % (
                    self.hwro.username,
                    name,
                )
    ...
    def addPosition(self):
        if self.hwro is not None:
            ...
            qrep = qt.QInputDialog.getText(
                "New Position", label_str, qt.QLineEdit.Normal, "", None
            )
...
class MultiplePositionConfigurator(qt.QDialog):
    def __init__(self, parent):
        ...
        general set button
        """
        self.geneSetButton = qt.QPushButton(
            "Store Current Motors Positions", self.frame
        )
        self.connect(self.geneSetButton, qt.SIGNAL("clicked()"), self.__geneSet)
    ...
    def setHardwareObject(self, hwro):
        ...
        if self.hwro is not None:
            for role, mot in self.hwro["motors"]._objectsByRole.iteritems():
                mne = role
                self.motorBrickList[mne] = MultiAxisAlignmentBrick(
                ...
                try:
                    ...
                    pass
                self.motorBrickList[mne]["hori. steps"] = (
                    xml_gui_step + "0.05 0.1 0.5 1"
                )
    ...
    def clearConfigurator(self):
        ...
        for conn in self.motorConnList:
            self.disconnect(conn, qt.PYSIGNAL("valueChanged"), self.__motorPosChanged)
        self.motorConnList = []
    ...
    def __geneSet(self):
        if self.hwro is not None:
            ...
            qt.QMessageBox.information(
                self,
                "Position alignment",
                "New position saved successfully.",
                qt.QMessageBox.Ok,
            )
...
class QubRadioGroup(qt.QFrame):
    ...
    def clearRadioList(self):
        for name, radio in self.buttonList.iteritems():
            self.disconnect(
                radio, qt.PYSIGNAL("PositionClicked"), self.__positionClicked
            )
            radio.close()
...
class MotorConnection(qt.QWidget):
    def __init__(self, motorObject, ind, role):
        ...
        self.motor.connect(
            self.motor, qt.PYSIGNAL("positionChanged"), self.positionChanged
        )
        self.motor.connect(self.motor, qt.PYSIGNAL("stateChanged"), self.stateChanged)

    def name(self):
        return self.__name

    def positionChanged(self, value):
        self.emit(qt.PYSIGNAL("ValueChanged"), (value, self.__name, self.index))

    def stateChanged(self, state):
        self.emit(qt.PYSIGNAL("StateChanged"), (state, self.__name, self.index))
...
class QubValue(qt.QWidget):
    ...
    def __init__(
        self, parent, titleType=Label, valueType=Label, orientation=Horizontal
    ):
        ...
        if self.titleType == QubValue.Combo:
            ...
            self.connect(
                self.titleWidget, qt.SIGNAL("activated(int)"), self.__activatedType
            )
        else:
        ...
        if self.valueType == QubValue.Combo:
            ...
            self.connect(
                self.valueWidget, qt.SIGNAL("activated(int)"), self.__activatedValue
            )
        elif self.valueType == QubValue.Text:
            self.valueWidget = qt.QLineEdit(self)
            self.connect(
                self.valueWidget, qt.SIGNAL("returnPressed()"), self.__returnPressed
            )
        else:
