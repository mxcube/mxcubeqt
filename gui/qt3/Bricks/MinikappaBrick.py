...
class MinikappaBrick(BlissWidget):
    ...
    def __init__(self, *args):
        ...
        QObject.connect(
            self.motorsList, SIGNAL("activated(const QString &)"), self.motorChanged
        )
        ...
        if enable:
            if self.kappaOnCmd is not None:
                try:
                    ...
                    logging.getLogger().exception(
                        "MinikappaBrick: problem enabling the minikappa for the sample changer"
                    )
            ...
            if self.kappaOffCmd is not None:
                try:
                    ...
                    logging.getLogger().exception(
                        "MinikappaBrick: problem disabling the minikappa from the sample changer"
                    )
    ...
    def addMotor(self, mnemonic, username, ho):
        ...
        def position_changed(position):
            return MinikappaBrick.positionChanged(self, username, position)

        def state_changed(state):
            return MinikappaBrick.stateChanged(self, username, state)

        self.__refs[ho] = (state_changed, position_changed)
        self.connect(ho, PYSIGNAL("stateChanged"), state_changed, instanceFilter=True)
        self.connect(
            ho, PYSIGNAL("positionChanged"), position_changed, instanceFilter=True
        )
    ...
    def setCommands(self, minikappa=None, disconnect=False):
        if disconnect:
            if self.kappaOnCmd is not None:
                ...
                self.kappaOnCmd.disconnectSignal(
                    "commandReplyArrived", self.kappaOnFinished
                )
                self.kappaOnCmd.disconnectSignal("commandFailed", self.kappaOnFinished)

            if self.kappaOffCmd is not None:
                self.kappaOffCmd.disconnectSignal(
                    "commandStarted", self.kappaOffStarted
                )
                self.kappaOffCmd.disconnectSignal(
                    "commandReplyArrived", self.kappaOffFinished
                )
                self.kappaOffCmd.disconnectSignal(
                    "commandFailed", self.kappaOffFinished
                )
            ...
            if minikappa is not None:
                try:
                    ...
                    logging.getLogger().warning(
                        "MinikappaBrick: unable to know if minikappa is beging used!"
                    )
                    self.kappaInUseChan = None
                else:
                    if self.kappaInUseChan is not None:
                        self.kappaInUseChan.connectSignal(
                            "update", self.kappaInUseChanged
                        )
                        self.kappaInUseChanged(self.kappaInUseChan.getValue())
            else:
                self.kappaInUseChan = None
