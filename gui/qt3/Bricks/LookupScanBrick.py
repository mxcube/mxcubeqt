...
class LookupScanBrick(BaseGraphicScan):
    def __init__(
        self, parent, name, uifile="LookupScan.ui", cmd_name="ltscan", **kwargs
    ):
        BaseGraphicScan.__init__(
            self, parent, name, uifile=uifile, cmd_name=cmd_name, **kwargs
        )
        ...
        qt.QObject.connect(
            table, qt.SIGNAL("valueChanged(int,int)"), self._valueChangedScanParam
        )
        ...
        for itemName, cbk in [
            ("Create new", self.__createPolygon),
        ...
        if property == "offsetmeasure":
            ...
            if ho is not None:
                try:
                    ...
                    self._ltinit.connectSignal(
                        "commandReplyArrived", self.__ltinitFinished
                    )
    ...
    def _updateGUI(self):
        if self._graphicSelection:
            ...
            if len(points) == 3 and self._XSize and self._YSize:
                ...
                try:
                    dist = math.sqrt(
                        ((points[0][0] - points[1][0]) * self._XSize) ** 2
                        + ((points[0][1] - points[1][1]) * self._YSize) ** 2
                    )
                    table.setText(
                        0, 2, self._formatString % (dist * self.__offsetMeasure)
                    )

                    dist = math.sqrt(
                        ((points[1][0] - points[2][0]) * self._YSize) ** 2
                        + ((points[1][1] - points[2][1]) * self._XSize) ** 2
                    )
                    table.setText(
                        1, 2, self._formatString % (dist * self.__offsetMeasure)
                    )
                    self.__refreshSteps()
                except BaseException:
                    import traceback
    ...
    def _valueChangedScanParam(self, row, column):
        ...
        if column == 0:
            ...
            if len(points) == 3:
                ...
                xori, yori = points[0]
                p = numpy.array(
                    [[xori, yori], [xori + width, yori], [xori + width, yori + height]]
                )
    ...
    def __ltinitFinished(self, *args):
        ...
        if posMot1:
            ...
            for motor_name, pos_motor in zip(
                [str(mot1Widget.currentText()), str(mot2Widget.currentText())],
                [posMot1, posMot2],
            ):
            ...
            self._SpecCmd(
                str(mot1Widget.currentText()), str(mot2Widget.currentText()), ctime
            )
    ...
    def __revertSelectedPolygon(self, nb):
        for item in self.__getIterator(True):
            ...
            if item.includeMode:
                pixmap = Icons.load("Plus2")
            else:
                pixmap = Icons.load("Minus2")
    ...
    def __createGridPoints(self):
        ...
        if self._graphicSelection.points():
            ...
            points = numpy.array(
                [
                    [x, y]
                    for y in numpy.linspace(points[0, 1], points[2, 1], nbAxis2 + 1)
                    for x in numpy.linspace(points[0, 0], points[1, 0], nbAxis1 + 1)
                ]
            )
    ...
    def __checkPolygoneItersection(self):
        ...
        if includePolygone:
            for points in includePolygone:
                ...
                if not allInPoints:
                    ...
                    allInPoints = [
                        x > 0 or y > 0 for x, y in itertools.izip(pInPoly, allInPoints)
                    ]
        ...
        for points in excludePolygone:
            pInPoly = polygone.points_inclusion(pointGrid, points, False)
            allInPoints = [
                x > 0 and not y > 0 for x, y in itertools.izip(allInPoints, pInPoly)
            ]
    ...
    def __setRegion(self, aCheckPolygoneFlag=True):
        ...
        for item in self.__getIterator():
            ...
            if item.includeMode:
                addRegion.append(region)
            else:
                minusRegion.append(region)
        self._graphicSelection.setRegion(addRegion, minusRegion)
        if aCheckPolygoneFlag:
            self.__checkPolygoneItersection()
