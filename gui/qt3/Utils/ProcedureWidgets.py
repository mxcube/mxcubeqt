# $Id: ProcedureWidgets.py,v 1.1 2004/08/10 10:05:20 guijarro Exp $
"""Contains a set of widgets designed to make things easier when creating panels, for example in Procedures
"""
...
class ProcedurePanel(QWidget):
    ...
    def setProcedure(self, proc):
        if proc is None:
            ...
            QObject.connect(
                HardwareRepository.emitter(self.procedure),
                PYSIGNAL("replyArrived"),
                self.replyArrived,
            )
            ...
            if self.procedure is not None:
                ...
                try:
                    ...
                    logging.getLogger().exception(
                        '%s: Cannot execute command "%s" on procedure %s',
                        str(self.name()),
                        cmd,
                        str(self.procedure.name()),
                    )
                else:
    ...
    def replyArrived(self, id, reply):
        ...
        if id == self.lastCommandID:
            ...
            if isinstance(reply, types.StringType) and reply.startswith("error:"):
                error = reply.split(":")[1]
                QMessageBox.warning(
...
class ProcedureEntryField(QVBox):
...
class FloatEntryField(ProcedureEntryField):
    ...
    def __init__(self, parent, caption="", unit=""):
        ...
        QObject.connect(
            self.textbox, SIGNAL("textChanged( const QString & )"), self.valueChanging
        )
        QObject.connect(self.cmdOK, SIGNAL("clicked()"), self.valueChanged)
        QObject.connect(self.cmdCancel, SIGNAL("clicked()"), self.cancelClicked)
...
class MotorPositionReminder(QTableItem):
    ...
    def setMotor(self, motor_mne):
        ...
        self.motor = HardwareRepository.getHardwareRepository().getHardwareObject(
            motor_mne
        )
    ...
    def motorPositionChanged(self, position):
        if not str(position).isalpha():
            ...
            self.cmdMotorPosition.setText(
                str(position).center(len(MotorPositionReminder.NOPOSITION))
            )
...
class ScanConfigurationTable(QTable):
    def __init__(self, parent):
