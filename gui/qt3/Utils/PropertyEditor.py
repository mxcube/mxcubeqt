...
class ConfigurationTable(QTable):
    ...
    def setPropertyBag(self, propertyBag, showHidden=False):
        ...
        if self.propertyBag is not None:
            ...
            for property in self.propertyBag:
                ...
                self.connect(
                    validationPanel.OK, SIGNAL("clicked()"), self.OnValidateClick
                )
                self.connect(
                    validationPanel.Cancel, SIGNAL("clicked()"), self.OnInvalidateClick
                )
                self.connect(
                    validationPanel.Reset, SIGNAL("clicked()"), self.OnResetClick
                )
    ...
    def setWidgetFromProperty(self, row, property):
        if property.getType() == "boolean":
            ...
            newPropertyItem = FileTableItem(
                self, property.getUserValue(), property.getFilter()
            )
            self.setItem(row, 1, newPropertyItem)
    ...
    def OnValueChanged(self, row, col):
        ...
        if not oldValue == property.getUserValue():
            self.emit(
                ...
                (property.getName(), oldValue, property.getUserValue()),
            )
...
class ColorTableItem(QTableItem):
    ...
    def createEditor(self):
        if self.cmdChangeColor is None:
            ...
            QObject.connect(
                self.cmdChangeColor, SIGNAL("clicked()"), self.cmdChangeColorClicked
            )
            QObject.connect(
                self.cmdResetColor, SIGNAL("clicked()"), self.cmdResetColorClicked
            )
