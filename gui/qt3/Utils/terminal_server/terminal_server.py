...
def execute_command():
    ...
    if len(python_code_to_execute) == 0:
        ...
        sys.__stdout__.write(
            "storing command %r for history for client %s\n"
...
def send_static_codemirror(filename):
    return bottle.static_file(
        filename, root=os.path.join(ROOT_PATH, "lib/CodeMirror-2.3/lib")
    )
...
def send_static_codemirror_py(filename):
    return bottle.static_file(
        filename, root=os.path.join(ROOT_PATH, "lib/CodeMirror-2.3/mode/python")
    )
