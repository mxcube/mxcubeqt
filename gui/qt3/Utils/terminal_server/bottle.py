...
"""
        if hasattr(plugin, "setup"):
            plugin.setup(self)
        if not isinstance(plugin, collections.Callable) and not hasattr(
            plugin, "apply"
        ):
...
"""
        ...
        try:
            ...
            if (
                isinstance(e, (KeyboardInterrupt, SystemExit, MemoryError))
        ...
        return self._cast(
            HTTPError(500, "Unsupported response type: %s" % type(first)),
            request,
            response,
        )
...
"""
        ...
        if NCTextIOWrapper:
            args["fp"] = NCTextIOWrapper(
                args["fp"], encoding="ISO-8859-1", newline="\n"
            )
...
"""
    ...
    def _get_status(self):
        depr(
            "BaseRequest.status will change to return a string in 0.11. Use"
...
class ServerAdapter:
    ...
    def __repr__(self):
        args = ", ".join(
            ["%s=%s" % (k, repr(v)) for k, v in list(self.options.items())]
        )
...
|\#.* # Comments
    ...
    def split_comment(cls, code):
        ...
        def subf(m):
            return "" if m.group(0)[0] == "#" else m.group(0)
