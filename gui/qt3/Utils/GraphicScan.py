...
class BaseGraphicScan(BlissWidget):
        ...
        if property == "horizontal" or property == "vertical":
            ...
            if ho is not None:
                ...
                if self._SpecCmd is not None:
                    ...
                    qt.QObject.connect(
                        self._SpecCmd, qt.PYSIGNAL("commandAborted"), self._scanAborted
                    )
                    qt.QObject.connect(
                        self._SpecCmd, qt.PYSIGNAL("commandFailed"), self._scanAborted
                    )
                try:
    ...
    def connectToView(self, view):
        ...
        if self._view is not None:
            ...
            showgButton.show()
            qt.QObject.connect(
                showgButton, qt.SIGNAL("toggled(bool)"), self._showGrabScan
            )
            ...
            self.connect(
                self._view, qt.PYSIGNAL("ForegroundColorChanged"), self._setColor
            )
    ...
    def _updateGUI(self):
        try:
            ...
            ####### HORIZONTAL #######
            if (
                self._XSize is not None
                and self._YSize is not None
                and self._beamx is not None
                and self._beamy is not None
            ):
                hmotor = self._getHorizontalMotor()
                try:
                    hStartPos = (
                        (hpixStart - self._beamx)
                        / hmotor.getProperty("unit")
                        * self._XSize
                    )
                    hStopPos = (
                        (hpixStop - self._beamx)
                        / hmotor.getProperty("unit")
                        * self._XSize
                    )
                ...
                try:
                    vStartPos = (
                        (vpixStart - self._beamy)
                        / vmotor.getProperty("unit")
                        * self._YSize
                    )
                    vStopPos = (
                        (vpixStop - self._beamy)
                        / vmotor.getProperty("unit")
                        * self._YSize
                    )
